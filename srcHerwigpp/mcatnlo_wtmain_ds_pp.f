     ...
     #  iunit,ifluxrwgt,ittintf,inonbtop,ievffmt
      character * 2 scheme
...
c
        ...
        xnorm = xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
        do jproc=loproc,maproc
          call xmcsubtwtpp(jproc,x1,x2,xm12,xm22,s,x,yi,cth1,cth2,
...
c
c
      subroutine xmcsubtwtpp(jproc,z1,z2,xm12,xm22,s,x,yi,cth1,cth2,
...
c For all the array outputs, (7,3,6) correspond to (idr,ileg,ie0sq)
     ...
     # q1q,q2q,e0sq,de0sqdx,de0sqdc,xz,zherppwt,xxkp,xqherppwt,
     ...
     # xjac,xjac_zqttoxywt,qk,dcosdcth1,
     # xmWt2,ptmin,ptmax,rrnd,fk88random,emscainv,emsca_bare,
...
c ptmin and ptmax define the veto region
      xmWt2=s*x/2.d0
      ptmin=max(ff1*sqrt(xmWt2),10.d0)
      ptmax=max(sqrt(xmWt2),ptmin+20.d0)
      rrnd=fk88random(ifk88seed)
     ...
     #    ss,tk,uk,q1q,q2q)
        do ie0sq=1,6
          if(ialwsplit(jproc,ileg,ie0sq).eq.1.and.flagxs(ileg))then
            ...
            xz=zherppwt(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                  tk,uk,q1q,q2q)
            xxkp=xqherppwt(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                    tk,uk,q1q,q2q)
            if(ileg.le.2)then
...
c associate it to the first non-zero ie0sq contribution
     ...
     #              tmp1=8*qin_kern(x,icode)/ss
                  ...
                  de0sqdx=dx2to2dx(ileg,ie0sq)
                  de0sqdc=dx2to2dc(ileg,ie0sq)
                  xjac=xjac_zqttoxywt(ileg,e0sq,de0sqdx,de0sqdc,
...
c  w2  = q1q-q2q-uk
c
      function zherppwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                 xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable z; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 zherppwt,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
...
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        zherppwt=zherpp(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        ...
        if(1-x.lt.tiny)then
          ...
          zherppwt=1-(1+cthg)*(1-x)/(1+xm12red-xm22red+beta)
        else
     ...
     #          ( (s-w1)*beta2*(2*s-(s-w1)*eps2+(s-w1)*beta2) )
        ...
        write(6,*)'zherppwt: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xqherppwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                  xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable ktilde; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 xqherppwt,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
...
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        xqherppwt=xqherpp(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        w1=-xq1q+xq2q-xtk
        w2=-xq2q+xq1q-xuk
        ...
        if(1-x.lt.tiny)then
          ...
          sth1=sqrt(1-cth1**2)
          cthg=yi*cth1+sqrt(1-yi**2)*cth2*sth1
          xqherppwt=(1+xm12red-xm22red+beta)*
     #              (1+xm12red-xm22red-beta*cthg)*s/
     #              (2.d0*(1+cthg))
        else
          z=zherppwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #               xtk,xuk,xq1q,xq2q)
        ...
        write(6,*)'xqherppwt: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xjac_zqttoxywt(ileg,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
...
c variables, and x and c are FKS variables
      implicit none 
      real * 8 xjac_zqttoxywt,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
     # x,yi,cth1,cth2,xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,tmp,xjac_zqttoxy,w1,w2,xm12red,xm22red,sth1,
     # beta,cthg,zmo,dktil0sfdc,z,zherppwt,xkp,xqherppwt,cpsip,spsip,
...
c outgoing parton #3 (top)
     ...
     #          ( (s-w1)*beta2*(2*s-(s-w1)*eps2+(s-w1)*beta2) )
        ...
        write(6,*)'xjac_zqttoxywt: unknown parton number'
        stop
      endif
      xjac_zqttoxywt=abs(tmp)
      return 
      end 
