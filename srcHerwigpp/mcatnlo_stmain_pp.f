     ...
     #  jwidth,inonbtop,mqeventsuw,ievffmt
      character * 2 scheme
...
c
        ...
        do jproc=loproc,maproc
          ileg=1
          call xmcsubtpp(jproc,ileg,x1,x2,xm12,s,x,yi,cth1,cth2,
...
c
        ...
        do jproc=loproco,maproco
          do ileg=3,4
            call xmcsubtpp(jproc,ileg,x1,x2,xm12,s,x,yi,zero,zero,
...
c In terms of momenta, the non-FKS outgoing parton has always label 5
      ...
      if(idec.eq.0)then
        ...
        elseif(ideconsh.eq.12)then
          if( abs(ip5).ge.11.and.abs(ip5).le.16 )then
            xmss(6) = xlep1mass(1)
            xmss(7) = xlep2mass(1)
          else
            xmss(6) = 0.d0
...
c
c
      subroutine xmcsubtpp(jproc,ileg,z1,z2,xm12,s,x,yi,cth1,cth2,yj,
...
c For all the array outputs, (8,2) correspond to (idr,ich)
     ...
     # q1q,q2q,gfactsf,gfactcl,z(8,2),xmatmc(8,2)
      real * 8 xm22,one,two,vcf,vtf,tiny,xlim1,sbar,tbar,zherpp,
     # xqherpp,gfunsoft,xfact,xjac,xjac_zqttoxy,ap,ap_kern,ap_kern_qc,
     # beta,xkpupp,xmcs
...
c the latter are are s=(p1+p2) and t=(p1-k1)
     ...
     #             e0sq,de0sqdx,de0sqdc)
      ...
      do ich=ichmin,ichmax
        do idr=1,idrmax(jproc,ich)
          ...
          if(ie0sq.ne.0)then
            z(idr,ich)=zherpp(ileg,e0sq(ie0sq),xm12,s,x,yi,yj,
     #                        tk,uk,q1q,q2q)
            xkp(idr,ich)=xqherpp(ileg,e0sq(ie0sq),xm12,s,x,yi,yj,
     #                           tk,uk,q1q,q2q)
            if(ileg.le.2)then
     ...
     #            ie0sq.eq.3.or.ie0sq.eq.6))then
              ...
              if(z(idr,ich).ge.0.d0.and.xkp(idr,ich).ge.0.d0.and. 
     #           xkp(idr,ich).le.(2*e0sq(ie0sq)))lzone(idr,ich)=.true.
            elseif(ileg.le.4)then
              ...
              beta=1-xm12/s
              if(ileg.eq.3)then
                if(ie0sq.eq.3.or.ie0sq.eq.5)then
                  write(*,*)'Error #4 in xmcsubtpp',ie0sq
                  stop
                endif
                if(ie0sq.eq.6)then
                  xkpupp=(s+xm12+s*beta)/2.d0
                else
                  xkpupp=2*e0sq(ie0sq)+xm12
                endif
              else
                if(ie0sq.eq.2.or.ie0sq.eq.4)then
                  write(*,*)'Error #5 in xmcsubtpp',ie0sq
                  stop
                endif
                if(ie0sq.eq.6)then
                  xkpupp=(s-xm12+s*beta)/2.d0
              ...
              if(z(idr,ich).ge.0.d0.and.xkp(idr,ich).ge.0.d0.and. 
     #           xkp(idr,ich).le.xkpupp)lzone(idr,ich)=.true.
            else
              write(*,*)'Error in xmcsubtpp: unknown leg',ileg
              stop
            endif
          endif
        enddo
...
c Compute MC subtraction terms 
     ...
     #                 de0sqdc(ie0sq),xm12,s,x,yi,yj,tk,uk,q1q,q2q)
              ...
              elseif(ileg.eq.3)then
                ...
                if(1-x.lt.tiny)then
                  ...
                  xjac=xjac_zqttoxy(ileg,e0sq(ie0sq),de0sqdx(ie0sq),
     #                 de0sqdc(ie0sq),xm12,s,x,yi,yj,tk,uk,q1q,q2q)
                  ap=ap_kern_qc(z(idr,ich),xkp(idr,ich),
     #                          xm12,abs(icode))/(1-z(idr,ich))
                  xlim1=2/s*xfact*xjac*ap/xkp(idr,ich)
                endif
     ...
     #                 de0sqdc(ie0sq),xm12,s,x,yi,yj,tk,uk,q1q,q2q)
              ...
              elseif(ileg.le.4)then
                write(*,*)'Error in xmcsubtpp: no final-state emission'
