     ...
     #  xmhl,xmhu,xmhl2,xmhu2,bwxmdpl,bwxmdmn,alsf,besf,wgtmax,
     #  hweight,xmb,gammaxeff,ptbconmin,ptbconmax
      integer 
     ...
     #  imcscale,ifk88seed,ip4,izero,ione,ievffmt,iprdct0hw,
     #  iuserun,idecmode,iabdvnotop
      character * 2 scheme
...
c Close LHEF header
     ...
     # '  <header>'
          write(ioutput,'(a)')
     # '  </header>'
c Dummy process number; the actual one can be entered above if need be
...
c sigma (pb) = 10^6 * sigma (mub)
     ...
     # ffunval5,ffun5pp,ytmp,zhwfct,zherpp_spl,x1t,x1soft,
     # x2t,x2soft,x1x2j,x1x2jac,zherpp_smn,xsum,dummy,xint,xitmp
...
c MC subtraction terms; ffunval5=0,1 --> dead zone, live zone
        ...
        if(ffunval5.ne.0.d0)then
          ...
          if(x1t.lt.1.and.x2t.lt.1)then
            ...
            do jproc=loproc,maproc
              jproc0=jproc
              call xmcsubtpp(jproc,s,xii,yi,ytmp,xmcsec)
              do i=1,4
                do itype=1,nl
                  vv(i,jproc,itype)=vv(i,jproc,itype)-
...
c The variables ionshell must be non zero here.
...
c 
      if(ionshell.eq.0)then
        write(6,*)'put_on_shell should not be called with',ionshell
        stop
      endif
      if(ionshell.eq.1.or.ionshell.eq.11)then
...
c
c
      subroutine xmcsubtpp(jproc,s,xii,y,ycnt,xmcxsec)
c Returns the Herwig++ subtraction terms. The output is xmcxsec.
c Call with ycnt=1 and ycnt=-1 for emissions from legs 1 and 2 
c respectively. The definition is
...
c dead zone by keeping y fixed
      ...
      integer jproc
      include 'hgscblks.h'
      real*8 tiny,vca,vcf,zero,ff1,alpha,x,xlim,xlim1,xlim2,pthw,
     # ptmin,ptmax,ptresc,emscwgt,emscafun,rrnd,fk88random,emscainv,
     # sbar,yd,ydeadpp,ay,xdeadpp,gfunsfpp,xfix,xfact,z,zherpp_spl,
     # xq2til,xqtherpp_spl,ap,ap_kern,xjac_qtzspl,zherpp_smn,
     # xqtherpp_smn,xjac_qtzsmn,gfact,xmcgg_spl,xmcgg_smn,born(4)
      integer ione,itwo,j,icode
      logical usexev
...
c
      if(abs(ycnt).ne.1.d0) then
        write(6,*)'xmcsubtpp called improperly: ycnt=',ycnt
        stop
      endif
      x=1-xii
      xlim=0.d0
      ...
      if(jproc.eq.1)then
        ...
        if(ycnt.eq.1.d0)then
          if(x.ge.xgsoftpp.and.x.le.xdeadpp(ay).and.y.ge.0.d0)then
            gfact=gfunsfpp(x,ay)
            xfix=xdeadpp(ay)
            xlim2=xmcgg_spl(s,xfix,y,vca,icode)
          endif
          if(y.gt.yd)xlim1=xmcgg_spl(s,x,y,vca,icode)
        elseif(ycnt.eq.-1.d0)then
          if(x.ge.xgsoftpp.and.x.le.xdeadpp(ay).and.y.lt.0.d0)then
            gfact=gfunsfpp(x,ay)
            xfix=xdeadpp(ay)
            xlim2=xmcgg_smn(s,xfix,y,vca,icode)
          endif
          if(y.lt.-yd)xlim1=xmcgg_smn(s,x,y,vca,icode)
        endif
        ...
        if(ycnt.eq.1.d0)then
          if(y.gt.yd)then
            ...
            elseif(1-y.lt.tiny)then
              xlim=(1-x)*(2-2*x+x**2)*(1+y)/x-
     #             (2-2*x-x**2+x**3)*(1-x)*(1-y**2)/(2*x**2)
              xlim=vcf*xlim
            else
              xfact=(1-x)*(1-y**2)
              z=zherpp_spl(x,y)
     ...
     #             (2-2*x-x**2+x**3)*(1-x)*(1-y**2)/(2*x**2)
        ...
        write(*,*)'xmcsubtpp: unknown process',jproc
        stop
      endif
      return
      end
...
c
      ...
      function xmcgg_spl(s,x,y,vca,icode)
c Contribution to MC subtraction terms, gg channel, emission from leg 1.
c See xmcsubtpp for comments on normalization
      implicit none
      real*8 xmcgg_spl,s,x,y,vca,tiny,xlim1,xfact,z,zherpp_spl,xq2til,
     # xqtherpp_spl,ap,ap_kern,xjac_qtzspl
      integer icode
      parameter (tiny=1.d-6)
c
      if(1-x.lt.tiny)then
        xlim1=2*vca*(1+x-y*(1-x))
      elseif(1-y.lt.tiny)then
        xlim1=2*(1-x+x**2)**2/x*(1+y)-
     #        (1-2*x)*(1-x**2*(1-x)**2)*(1-y**2)/x**2
        xlim1=vca*xlim1
      else
        xfact=(1-x)*(1-y**2)
        z=zherpp_spl(x,y)
      ...
      function xmcgg_smn(s,x,y,vca,icode)
c Contribution to MC subtraction terms, qq channel, emission from leg 2.
c See xmcsubtpp for comments on normalization
      implicit none
      real*8 xmcgg_smn,s,x,y,vca,tiny,xlim1,xfact,z,zherpp_smn,xq2til,
...
c
      if(x.lt.0.d0.or.x.gt.1.d0)then
        write(6,*)'Fatal error in emscafun',x
        stop
      endif
...
c
      if(r.lt.0.d0.or.r.gt.1.d0.or.alpha.ne.1.d0)then
        write(6,*)'Fatal error in emscainv',r,alpha
        stop
      endif
