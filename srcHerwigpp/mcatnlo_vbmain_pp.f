...
c events are indentified with letters a and b respectively
     ...
     # alsf,besf,wgtmax,xsecpp,xerrpp
     ...
     # ianomcpl,ianomsvt,icplwgt,ixxsave,icplsave,ievffmt,isubttype
...
c Here, assume that the unweighting efficiency is larger than 10%
      maxtrials=10*maxevt
c Although this code uses zeta subtraction, set isubttype=0 (which usually
c correspond to standard subtraction) to use setgfpar() as in other codes
      isubttype=0
...
c     Close LHEF header
              if(idec.eq.0)then
                xsecpp=avtot*brrv1msb*brrv2msb
                xerrpp=dtot*brrv1msb*brrv2msb
              else
                xsecpp=avtot
                xerrpp=dtot
              endif
     ...
     #             '  <header>'
              write(ioutput,'(a)')
     #             '  </header>'
              call write_lhef_init(ioutput,abs(itmpvv),ecm,
...
c sigma (pb) = 10^6 * sigma (mub)
     ...
     #  xfact,ww3up,ww3do,ffunval5,ffun5pp,zgmu2_mc,ytmp,zhwfct,
     #  zherpp_spl,x1t,x1soft,x2t,x2soft,x1x2j,x1x2jac,zherpp_smn,xsum,
     #  dummy,xint,xtmp
        ...
        if(ffunval5.ne.0.d0)then
          ...
          if(prc.eq.'qq') then
            ...
            if(x1t.lt.1.and.x2t.lt.1)then
              ...
              call xmcsubtpp(s,x,y,cth1,cth2,x1,x2,ytmp,ww3up,ww3do)
...
c sigma (pb) = 10^6 * sigma (mub)
      ...
      real * 8 x,y,cth1,th2,cth2,sx,ro,b,rox,bx,rolimx,blimx,btildex,
     #  rotildx,xktrel,csi,rx,x1,x2,xnorm,xnormc,xnormsv,xnormborn,
     #  xlgomx,ffunval5,ffun5pp,tk,uk,q1q,q2q,v1a,v1b,v1c,v3a,v3b,
     #  v3c,v13,zg2,zgmu2_mc,xfact,ytmp,zhwfct,zherpp_spl,x1t,x1soft,
     #  x2t,x2soft,x1x2j,x1x2jac,ww3up,ww3do,zherpp_smn,zgmu2_nlo,
...
c
c
      subroutine xmcsubtpp(s,x,y,cth1,cth2,x1,x2,ycnt,ww3up,ww3do)
...
c since the real emission matrix elements are also multiplied by 4*tk*uk.
     ...
     # one,cth1bar,ro,beta,tk,uk,q1q,q2q,v1a,v1b,v1c,v3a,v3b,
     # v3c,v13,dkswzborn,xsecup,xsecdo
      real*8 ubar,brnup,brndo
      integer ione,itwo,icode,i,j
...
c
     ...
     #   prdct.ne.'z '.and.prdct.ne.'ww')then
        write(6,*)'xmcsubtpp: process not implemented',prdct
        stop
      endif
      ...
      xlim1=0.d0
      xlim2=0.d0
c Insert here stuff relevant to sliding emsca if need be
...
c Compute the process-independent factors
     ...
     #             (1-3*x+2*x**2)*(1-x)*(1-y**2)
        ...
        xlim=4*s*xlim
         ...
         write(*,*)'xmcsubtpp: unknown process',prc
         stop
      endif
      ww3up = xlim*brnup
      ww3do = xlim*brndo
...
c Note that xscale=xfact*emsca here (internal variables are in TeV)
         dummy=0.d0
         DO I=1,3
            DO J=1,4
