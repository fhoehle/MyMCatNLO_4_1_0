     ...
     #  iunit,ifluxrwgt,ittintf,inonbtop,i2hdmtype
      character * 2 scheme
...
c Run DR if mH>mtop
      if(xm2.ge.xm1)then
        write(*,*)'Please use DR if mH>mtop'
        stop
      endif
c
      write(*,*)' '
...
c
        ...
        xnorm = xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
        do jproc=loproc,maproc
          call xmcsubtht(jproc,x1,x2,xm12,xm22,s,x,yi,cth1,cth2,
...
c
     ...
     #                      tk,uk,q1q,q2q,xinv)
              ...
              if(jproc.ne.3)then
                call frealhtme(s,x,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                         jproc,vecmccl)
              else
...
c
     ...
     #                      tk,uk,q1q,q2q,xinv)
              ...
              xnorm = xnorm * (1-gfsf(jproc))
              if(jproc.ne.3)then
                call frealhtme(sx,xtmp,yi,cth2,tk,uk,q1q,q2q,xinv,
     #                         jproc,vecmcsf)
              else
...
c
     ...
     #                        tk,uk,q1q,q2q,xinv)
                ...
                xnorm = - xnorm * (1-gfsf(jproc))
                if(jproc.ne.3)then
                  call frealhtme(sx,xtmp,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                           jproc,vecmcsc)
                else
...
c the exact real and Born lepton matrix elements
      implicit none
      real*8 pi,tolerance,bdredfact,half
      parameter (pi=3.14159265358979312D0)
...
c tH cross section. May become a process-dependent correction if need be.
     ...
     # rat1,qphsp,q1,tk,uk,q1q,q2q,xdec,xmadevht_dc,unxdec,xsngltht,
     # dmfactb1,dmfact1,phspfact1,xboundb,rat,
     # xmadsubht_dc,xsnglhtsub,x1p,x2p,
...
c kinematics for decayed and undecayed matrix elements
     ...
     #                  xtq,xbq,xtl,xtn,1)
        ...
        xdec=xmadevht_dc(iborn,jjprc,jidr,ps,tk,uk,xmom_cm)
...
c computation of spin correlations
        xdec=xmadsubht_dc(iborn,jjprc,jidr,xm12,xm22,ps,xtmp,yitmp,
     #                    pcth1,pcth2,tk,uk,q1q,q2q,half,half,x1p,x2p,
     #                    q1,cthtq1,phitq1,cthtq2,phitq2)
        unxdec=xsnglhtsub(iborn,jjprc,jidr,xm12,xm22,ps,xtmp,yitmp,
     #                    pcth1,pcth2,tk,uk,q1q,q2q,half,half,x1p,x2p)
      else
...
c
      if(iborn.eq.0)then
        ...
        if(jproc.ne.3)then
          call frealhtme(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,jproc,xmatin)
        else
      ...
      function xsnglhtsub(iborn,jproc,idr,xm12,xm22,s,x,yi,cth1,cth2,
     #                    tk,uk,q1q,q2q,x1,x2,x1p,x2p)
c Wrapper for the undecayed matrix elements of the original code.
c For Born matrix elements, q1q is t (consistently with invar_in)
      implicit none
      real*8 xsnglhtsub,xm12,xm22,s,x,yi,cth1,cth2,tk,uk,q1q,q2q,
     # x1,x2,x1p,x2p
      integer iborn,jproc,idr
      real*8 xmatin(7)
c
      if(iborn.eq.0)then
        write(6,*)'Error #1 in xsnglhtsub'
        stop
      else
     ...
     #      jproc.eq.3 )then
          write(6,*)'Error #2 in xsnglhtsub:',jproc,idr
          stop
        endif
        call frealhtsub(jproc,xm12,xm22,s,x,yi,cth1,cth2,
     #                  tk,uk,q1q,q2q,x1,x2,x1p,x2p,xmatin)
      endif
      xsnglhtsub=xmatin(idr)
      return
      end
...
c
      ...
      implicit none
      include 'stpcblks.h'
      real * 8 pi,zero,one,aem,cthw2,sthw2,xmw,gaw,xmt,twidth,xia,xib
      real * 8 ruckm,rcckm,rtckm,rducckm,rsucckm,rbucckm
      parameter (pi=3.14159265358979312D0)
      parameter (one=1.d0)
      parameter (zero=0.d0)
...
c
c
      subroutine xmcsubtht(jproc,z1,z2,xm12,xm22,s,x,yi,cth1,cth2,
...
c For all the array outputs, (7,3,6) correspond to (idr,ileg,ie0sq)
     ...
     # q1q,q2q,e0sq,de0sqdx,de0sqdc,xz,zherwht,xxi,xiherwht,
     ...
     # xjac,xjac_xiztoxyht,qk,dcosdcth1,
     ...
     #    ss,tk,uk,q1q,q2q)
        do ie0sq=1,6
          if(ialwsplit(jproc,ileg,ie0sq).eq.1.and.flagxs(ileg))then
            ...
            xz=zherwht(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                 tk,uk,q1q,q2q)
            xxi=xiherwht(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                   tk,uk,q1q,q2q)
            if(ileg.le.2)then
...
c associate it to the first non-zero ie0sq contribution
     ...
     #              tmp1=8*qin_kern(x,icode)/ss
                  ...
                  de0sqdx=dx2to2dx(ileg,ie0sq)
                  de0sqdc=dx2to2dc(ileg,ie0sq)
                  xjac=xjac_xiztoxyht(ileg,e0sq,de0sqdx,de0sqdc,
...
c  w2  = q1q-q2q-uk
c
      function zherwht(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                 xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable z; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 zherwht,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
...
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        zherwht=zherw(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
     ...
     #          ( (s-w1)*beta2*(2*s-(s-w1)*eps2+(s-w1)*beta2) )
          zherwht=1-tbeta1*zeta1-w1/(2*(1+tbeta1)*e0sq)
        endif
      else
        write(6,*)'zherwht: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xiherwht(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                  xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable xi; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 xiherwht,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,xiherw,w1,beta,tbeta10,sth1,cthg,z,zherwht
      parameter (tiny=1.d-3)
      parameter (dummy=-1.d3)
c
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        xiherwht=xiherw(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        w1=-xq1q+xq2q-xtk
        if(e0sq.le.(w1+xm12))then
        ...
        if(1-x.lt.tiny)then
          ...
          sth1=sqrt(1-cth1**2)
          cthg=yi*cth1+sqrt(1-yi**2)*cth2*sth1
          xiherwht=( s**2*(1+beta)*(1-beta*cthg)*(1+tbeta10) +
     ...
     #         (xm12-xm22)*(xm12-xm22+s*(2+beta*(1-cthg))) )
          ...
          z=zherwht(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #              xtk,xuk,xq1q,xq2q)
          if(z.gt.0.d0)then
            xiherwht=w1/(2*z*(1-z)*e0sq)
          else
            xiherwht=-1.d0
          endif
        endif
      else
        write(6,*)'xiherwht: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xjac_xiztoxyht(ileg,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
...
c variables, and x and c are FKS variables
      implicit none 
      real * 8 xjac_xiztoxyht,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
     # x,yi,cth1,cth2,xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,tmp,xjac_xiztoxy,w1,w2,sth1,beta,tbeta10,
     # cthg,zmo,z,zherwht,xi,xiherwht,cpsip,spsip,beta2,tbeta1,
...
c outgoing parton #3 (top)
     ...
     #              xtk,xuk,xq1q,xq2q)
          if(z.gt.0.d0)then
            xi=xiherwht(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                  xtk,xuk,xq1q,xq2q)
     ...
     #            )*tbeta1/( 2*e0sq*(1+tbeta1)*z*(1-z) )
        ...
        write(6,*)'xjac_xiztoxyht: unknown parton number'
        stop
      endif
      xjac_xiztoxyht=abs(tmp)
      return 
      end 
