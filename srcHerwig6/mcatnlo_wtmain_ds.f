      PROGRAM MCATNLO_WTMAIN_DS
      implicit none
      include 'stpcblks.h'
      real * 8 value(20),xmass(-5:21),xmomshifts(4),vickm(1:6,1:6),
     #  xlep1mass(2),xlep2mass(2),alsfwt(3),besfwt(3),alclwt(3),
     #  beclwt(3),alazi(3),beazi(3)
      real * 8 pi,xalsfwt,xbesfwt,xalclwt,xbeclwt,xalazi,xbeazi,
     ...
     #  iunit,ifluxrwgt,ittintf,inonbtop
      character * 2 scheme
      ...
      logical evgen
      external sig5azw_wt,sig5bzw_wt
      parameter (pi=3.14159265358979312D0)
      parameter (xmone=-1.d0)
      parameter (izero=0)
      parameter (ione=1)
c
c common /strfun0/ is only in strfunwt:
c ndns = pdf type
...
c CKM matrix elements entered by the user
      common/cvickm/vickm
c alsfwt and besfwt are the parameters entering gfunsoftwt
      common/cgfunsfwt/alsfwt,besfwt
c alclwt and beclwt are the parameters entering gfuncoll
      common/cgfunclwt/alclwt,beclwt
...
c ifluxrwgt=0 ==> reweights the parton flux for ttbar subtraction terms
c ifluxrwgt=1 ==> no reweighting
      common/cifluxrwgt/ifluxrwgt
...
c----------------------------------------------------------
c Decay variables
c Decay of the top and W: idec=0    -->   top and W decay
c                         idec=1    -->   top and W don't decay
      common/cidec/idec
c Mass ranges: jwidth=0    -->   top and hard W on shell
c              jwidth=1    -->   top and hard W off shell
...
c il1hw is relevant to the W from the top, il2hw to the W from the hard process
...
c reweight factors when top and W decay, that include branching ratios
...
c Production channels: ichmin<=ich<=ichmax, ich=1,2,3 => s-, t-, Wt-channel
...
c Flag to put top and W decay products on shell
...
c Select process; enter HERWIG code, and converts to NLO codes
      ...
      write(*,*)'      -(1)2030-IT for Wt-channel'
     ...
     #  '! 0/1/4 -> t+tb/tb/t'
      ...
      if(ichmin.ne.3)then
        write(*,*)'Use this code for Wt-channel only'
        stop
      endif
c
      write(*,*)' '
      ...
      xm12 = xm1**2
      xmt = xm1
c Mass of the W, and its width (enters propagators, and top decay). A lot
c of redundancy for backward compatibility and possible merging with s-
c and t-channel
      write(*,*)' '
...
c Top and W decay parameters
     ...
     #  '! 0..6 -> t dec, 7 -> t undec'
      write(*,*)' '
      write(*,*)'Enter IL=0..6 for W->d1_IL d2_IL'
      write(*,*)'      IL=7 for undecayed W'
      read(*,*) il2hw
      write(11,'(1x,i2,31x,a)') il2hw,
     #  '! 0..6 -> W dec, 7 -> W undec'
      if( (il1hw.eq.7.and.il2hw.ne.7) .or.
     #    (il1hw.ne.7.and.il2hw.eq.7) )then
        write(*,*) 'Top and W must both decay or being stable'
        stop
...
c W from the hard process
        write(*,*)' '
        write(*,*)'Enter GammaX, M_V2(min), M_V2(max) for hard W'
        write(*,*)
     #   '  If GammaX>0, the boson mass is chosen in the range'
        write(*,*)'      M0-GammaX*width < M_W < M0+GammaX*width'
...
c Do not set twidth=0 here, since top width must be different from zero,
c being used in the ttbar subtraction by MadEvent routines
        if(twidth.eq.0.d0)then
          write(*,*)'Diagram subtraction needs non-zero top width'
          stop
        elseif(twidth.lt.0.d0.or.gaw.lt.0.d0)then
...
c-----------------------------------------------------------------
     ...
     #   'Enter alpha and beta for G_soft [bg]'
        ...
        read(*,*) xalsfwt,xbesfwt
        write(11,'(2(1x,d10.4),12x,a)') xalsfwt,xbesfwt,
     #    '! alpha, beta [soft;bg]'
      else
        xalsfwt=1.d0
        xbesfwt=-0.1d0
     ...
     #   'Enter alpha and beta for G_coll [bg]'
        ...
        write(*,*)' Allowed ranges: alpha>=1, 0<|beta|<=1'
        read(*,*) xalclwt,xbeclwt
        write(11,'(2(1x,d10.4),12x,a)') xalclwt,xbeclwt,
     #    '! alpha, beta [coll;bg]'
      else
        xalclwt=0.d0
        xbeclwt=0.d0
...
c 
      ...
      if(iinput.eq.1)then
        ...
        write(*,*)'Enter 1 to set mu_0^2=(M_t^2+pt_t^2+M_W^2+pt_W^2)/2'
        write(*,*)'      2 to set mu_0^2=M_t^2'
        write(*,*)'      3 to set mu_0^2=(M_t^2+M_W^2)/2'
...
c
      if(iinput.eq.1)then
        write(*,*)' '
        write(*,*)'Enter 0 for flux reweighting'
        write(*,*)'      1 otherwise'
        read(*,*)ifluxrwgt
        write(11,'(1(1x,i8),25x,a)') ifluxrwgt,
     #    '! 0=flux rwgt, 1=no reweighting'
      else
        ifluxrwgt=0
      endif
      if(ifluxrwgt.ne.0.and.ifluxrwgt.ne.1)then
        write(*,*)'No such option for flux reweighting'
        stop
      endif
c---------------------------------------------------------------
...
c When top and W decay, compute the relevant parameters
      if(idec.eq.0)then
        if( (gammay1.ne.0.d0.and.twidth.eq.0.d0) .or.
     #      (gammax2.ne.0.d0.and.gaw.eq.0.d0) )then
          write(*,*)'Non-zero mass ranges require non-zero widths'
          stop
        endif
        if(gammay1.eq.0.and.gammax2.eq.0)then
          jwidth=0
          ym1low2=-1.d0
          ym1upp2=-1.d0
          tbw1delf=0.d0
          ...
          xm2upp2=-1.d0
          bw2delf=0.d0
        elseif(gammay1.ne.0.and.gammax2.ne.0)then
...
c
        ...
        call run_bases(sig5azw_wt,fnamea,ndim,nwild,ncl3,it1,it2,
...
c
        ...
        call run_bases(sig5bzw_wt,fnameb,ndim,nwild,ncl3,it1,it2,
...
c Write all the quantities which identify the run
 ...
 111    continue
     ...
     #            xbrrtophad
        ...
        write(*,*)' BR(W -> [udbar+usbar+ubbar])= ',xbrrwhad
        write(*,*) '   '
...
c rbucckm = |V_ub|^2+|V_cb|^2
      ...
      integer ih1,ih2,ndns1,ndns2,ii,jproc,itype,ittbar,jwt
      parameter(pi=3.14159265358979312D0)
      parameter (jwt=3)
      include 'stpcblks.h'
      common/strfun0/ih1,ih2,ndns1,ndns2
...
c jproc=1
      if(ittmin.eq.1)then
c t production
        sf(1,1,1,1)=dble(fh1x1( 0) * fh2x2( 0))*rtckm
      endif
      if(ittmax.eq.2)then
c tbar production
        sf(1,1,1,2)=dble(fh1x1( 0) * fh2x2( 0))*rtckm
      endif
c jproc=2
      if(ittmin.eq.1)then
c t production
        sf(1,2,1,1)=dble(fh1x1( 1) * fh2x2(-1))*rtckm
        sf(1,2,2,1)=dble(fh1x1( 4) * fh2x2(-4))*rtckm
        sf(1,2,3,1)=dble(fh1x1( 2) * fh2x2(-2))*(rtckm-ckm2(6,2))
        sf(1,2,4,1)=dble(fh1x1( 3) * fh2x2(-3))*(rtckm-ckm2(6,3))
c
        sf(2,2,1,1)=dble(fh1x1(-1) * fh2x2( 1))*rtckm
        sf(2,2,2,1)=dble(fh1x1(-4) * fh2x2( 4))*rtckm
        sf(2,2,3,1)=dble(fh1x1(-2) * fh2x2( 2))*(rtckm-ckm2(6,2))
        sf(2,2,4,1)=dble(fh1x1(-3) * fh2x2( 3))*(rtckm-ckm2(6,3))
c
        sf(3,2, 1,1)=dble(fh1x1( 2) * fh2x2( 1))*ckm2(6,2)
        sf(3,2, 2,1)=dble(fh1x1( 2) * fh2x2( 4))*ckm2(6,2)
        sf(3,2, 3,1)=dble(fh1x1( 2) * fh2x2(-1))*ckm2(6,2)
        sf(3,2, 4,1)=dble(fh1x1( 2) * fh2x2(-4))*ckm2(6,2)
        sf(3,2, 5,1)=dble(fh1x1( 3) * fh2x2( 1))*ckm2(6,3)
        sf(3,2, 6,1)=dble(fh1x1( 3) * fh2x2( 4))*ckm2(6,3)
        sf(3,2, 7,1)=dble(fh1x1( 3) * fh2x2(-1))*ckm2(6,3)
        sf(3,2, 8,1)=dble(fh1x1( 3) * fh2x2(-4))*ckm2(6,3)
        sf(3,2, 9,1)=dble(fh1x1( 5) * fh2x2( 1))*ckm2(6,5)
        sf(3,2,10,1)=dble(fh1x1( 5) * fh2x2( 4))*ckm2(6,5)
        sf(3,2,11,1)=dble(fh1x1( 5) * fh2x2(-1))*ckm2(6,5)
        sf(3,2,12,1)=dble(fh1x1( 5) * fh2x2(-4))*ckm2(6,5)
        sf(3,2,13,1)=dble(fh1x1( 2) * fh2x2( 3))*ckm2(6,2)
        sf(3,2,14,1)=dble(fh1x1( 2) * fh2x2( 5))*ckm2(6,2)
        sf(3,2,15,1)=dble(fh1x1( 2) * fh2x2(-3))*ckm2(6,2)
        sf(3,2,16,1)=dble(fh1x1( 2) * fh2x2(-5))*ckm2(6,2)
        sf(3,2,17,1)=dble(fh1x1( 3) * fh2x2( 2))*ckm2(6,3)
        sf(3,2,18,1)=dble(fh1x1( 3) * fh2x2( 5))*ckm2(6,3)
        sf(3,2,19,1)=dble(fh1x1( 3) * fh2x2(-2))*ckm2(6,3)
        sf(3,2,20,1)=dble(fh1x1( 3) * fh2x2(-5))*ckm2(6,3)
        sf(3,2,21,1)=dble(fh1x1( 5) * fh2x2( 2))*ckm2(6,5)
        sf(3,2,22,1)=dble(fh1x1( 5) * fh2x2( 3))*ckm2(6,5)
        sf(3,2,23,1)=dble(fh1x1( 5) * fh2x2(-2))*ckm2(6,5)
        sf(3,2,24,1)=dble(fh1x1( 5) * fh2x2(-3))*ckm2(6,5)
c
        sf(4,2, 1,1)=dble(fh1x1( 1) * fh2x2( 2))*ckm2(6,2)
        sf(4,2, 2,1)=dble(fh1x1( 4) * fh2x2( 2))*ckm2(6,2)
        sf(4,2, 3,1)=dble(fh1x1(-1) * fh2x2( 2))*ckm2(6,2)
        sf(4,2, 4,1)=dble(fh1x1(-4) * fh2x2( 2))*ckm2(6,2)
        sf(4,2, 5,1)=dble(fh1x1( 1) * fh2x2( 3))*ckm2(6,3)
        sf(4,2, 6,1)=dble(fh1x1( 4) * fh2x2( 3))*ckm2(6,3)
        sf(4,2, 7,1)=dble(fh1x1(-1) * fh2x2( 3))*ckm2(6,3)
        sf(4,2, 8,1)=dble(fh1x1(-4) * fh2x2( 3))*ckm2(6,3)
        sf(4,2, 9,1)=dble(fh1x1( 1) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,10,1)=dble(fh1x1( 4) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,11,1)=dble(fh1x1(-1) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,12,1)=dble(fh1x1(-4) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,13,1)=dble(fh1x1( 3) * fh2x2( 2))*ckm2(6,2)
        sf(4,2,14,1)=dble(fh1x1( 5) * fh2x2( 2))*ckm2(6,2)
        sf(4,2,15,1)=dble(fh1x1(-3) * fh2x2( 2))*ckm2(6,2)
        sf(4,2,16,1)=dble(fh1x1(-5) * fh2x2( 2))*ckm2(6,2)
        sf(4,2,17,1)=dble(fh1x1( 2) * fh2x2( 3))*ckm2(6,3)
        sf(4,2,18,1)=dble(fh1x1( 5) * fh2x2( 3))*ckm2(6,3)
        sf(4,2,19,1)=dble(fh1x1(-2) * fh2x2( 3))*ckm2(6,3)
        sf(4,2,20,1)=dble(fh1x1(-5) * fh2x2( 3))*ckm2(6,3)
        sf(4,2,21,1)=dble(fh1x1( 2) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,22,1)=dble(fh1x1( 3) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,23,1)=dble(fh1x1(-2) * fh2x2( 5))*ckm2(6,5)
        sf(4,2,24,1)=dble(fh1x1(-3) * fh2x2( 5))*ckm2(6,5)
c
        sf(5,2,1,1)=dble(fh1x1( 2) * fh2x2(-2))*ckm2(6,2)
        sf(5,2,2,1)=dble(fh1x1( 3) * fh2x2(-3))*ckm2(6,3)
        sf(5,2,3,1)=dble(fh1x1( 5) * fh2x2(-5))*ckm2(6,5)
c
        sf(6,2,1,1)=dble(fh1x1(-2) * fh2x2( 2))*ckm2(6,2)
        sf(6,2,2,1)=dble(fh1x1(-3) * fh2x2( 3))*ckm2(6,3)
        sf(6,2,3,1)=dble(fh1x1(-5) * fh2x2( 5))*ckm2(6,5)
c
        sf(7,2,1,1)=dble(fh1x1( 2) * fh2x2( 2))*ckm2(6,2)
        sf(7,2,2,1)=dble(fh1x1( 3) * fh2x2( 3))*ckm2(6,3)
        sf(7,2,3,1)=dble(fh1x1( 5) * fh2x2( 5))*ckm2(6,5)
      endif
      if(ittmax.eq.2)then
c tbar production
        sf(1,2,1,2)=dble(fh1x1(-1) * fh2x2( 1))*rtckm
        sf(1,2,2,2)=dble(fh1x1(-4) * fh2x2( 4))*rtckm
        sf(1,2,3,2)=dble(fh1x1(-2) * fh2x2( 2))*(rtckm-ckm2(6,2))
        sf(1,2,4,2)=dble(fh1x1(-3) * fh2x2( 3))*(rtckm-ckm2(6,3))
c
        sf(2,2,1,2)=dble(fh1x1( 1) * fh2x2(-1))*rtckm
        sf(2,2,2,2)=dble(fh1x1( 4) * fh2x2(-4))*rtckm
        sf(2,2,3,2)=dble(fh1x1( 2) * fh2x2(-2))*(rtckm-ckm2(6,2))
        sf(2,2,4,2)=dble(fh1x1( 3) * fh2x2(-3))*(rtckm-ckm2(6,3))
c
        sf(3,2, 1,2)=dble(fh1x1(-2) * fh2x2(-1))*ckm2(6,2)
        sf(3,2, 2,2)=dble(fh1x1(-2) * fh2x2(-4))*ckm2(6,2)
        sf(3,2, 3,2)=dble(fh1x1(-2) * fh2x2( 1))*ckm2(6,2)
        sf(3,2, 4,2)=dble(fh1x1(-2) * fh2x2( 4))*ckm2(6,2)
        sf(3,2, 5,2)=dble(fh1x1(-3) * fh2x2(-1))*ckm2(6,3)
        sf(3,2, 6,2)=dble(fh1x1(-3) * fh2x2(-4))*ckm2(6,3)
        sf(3,2, 7,2)=dble(fh1x1(-3) * fh2x2( 1))*ckm2(6,3)
        sf(3,2, 8,2)=dble(fh1x1(-3) * fh2x2( 4))*ckm2(6,3)
        sf(3,2, 9,2)=dble(fh1x1(-5) * fh2x2(-1))*ckm2(6,5)
        sf(3,2,10,2)=dble(fh1x1(-5) * fh2x2(-4))*ckm2(6,5)
        sf(3,2,11,2)=dble(fh1x1(-5) * fh2x2( 1))*ckm2(6,5)
        sf(3,2,12,2)=dble(fh1x1(-5) * fh2x2( 4))*ckm2(6,5)
        sf(3,2,13,2)=dble(fh1x1(-2) * fh2x2(-3))*ckm2(6,2)
        sf(3,2,14,2)=dble(fh1x1(-2) * fh2x2(-5))*ckm2(6,2)
        sf(3,2,15,2)=dble(fh1x1(-2) * fh2x2( 3))*ckm2(6,2)
        sf(3,2,16,2)=dble(fh1x1(-2) * fh2x2( 5))*ckm2(6,2)
        sf(3,2,17,2)=dble(fh1x1(-3) * fh2x2(-2))*ckm2(6,3)
        sf(3,2,18,2)=dble(fh1x1(-3) * fh2x2(-5))*ckm2(6,3)
        sf(3,2,19,2)=dble(fh1x1(-3) * fh2x2( 2))*ckm2(6,3)
        sf(3,2,20,2)=dble(fh1x1(-3) * fh2x2( 5))*ckm2(6,3)
        sf(3,2,21,2)=dble(fh1x1(-5) * fh2x2(-2))*ckm2(6,5)
        sf(3,2,22,2)=dble(fh1x1(-5) * fh2x2(-3))*ckm2(6,5)
        sf(3,2,23,2)=dble(fh1x1(-5) * fh2x2( 2))*ckm2(6,5)
        sf(3,2,24,2)=dble(fh1x1(-5) * fh2x2( 3))*ckm2(6,5)
c
        sf(4,2, 1,2)=dble(fh1x1(-1) * fh2x2(-2))*ckm2(6,2)
        sf(4,2, 2,2)=dble(fh1x1(-4) * fh2x2(-2))*ckm2(6,2)
        sf(4,2, 3,2)=dble(fh1x1( 1) * fh2x2(-2))*ckm2(6,2)
        sf(4,2, 4,2)=dble(fh1x1( 4) * fh2x2(-2))*ckm2(6,2)
        sf(4,2, 5,2)=dble(fh1x1(-1) * fh2x2(-3))*ckm2(6,3)
        sf(4,2, 6,2)=dble(fh1x1(-4) * fh2x2(-3))*ckm2(6,3)
        sf(4,2, 7,2)=dble(fh1x1( 1) * fh2x2(-3))*ckm2(6,3)
        sf(4,2, 8,2)=dble(fh1x1( 4) * fh2x2(-3))*ckm2(6,3)
        sf(4,2, 9,2)=dble(fh1x1(-1) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,10,2)=dble(fh1x1(-4) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,11,2)=dble(fh1x1( 1) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,12,2)=dble(fh1x1( 4) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,13,2)=dble(fh1x1(-3) * fh2x2(-2))*ckm2(6,2)
        sf(4,2,14,2)=dble(fh1x1(-5) * fh2x2(-2))*ckm2(6,2)
        sf(4,2,15,2)=dble(fh1x1( 3) * fh2x2(-2))*ckm2(6,2)
        sf(4,2,16,2)=dble(fh1x1( 5) * fh2x2(-2))*ckm2(6,2)
        sf(4,2,17,2)=dble(fh1x1(-2) * fh2x2(-3))*ckm2(6,3)
        sf(4,2,18,2)=dble(fh1x1(-5) * fh2x2(-3))*ckm2(6,3)
        sf(4,2,19,2)=dble(fh1x1( 2) * fh2x2(-3))*ckm2(6,3)
        sf(4,2,20,2)=dble(fh1x1( 5) * fh2x2(-3))*ckm2(6,3)
        sf(4,2,21,2)=dble(fh1x1(-2) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,22,2)=dble(fh1x1(-3) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,23,2)=dble(fh1x1( 2) * fh2x2(-5))*ckm2(6,5)
        sf(4,2,24,2)=dble(fh1x1( 3) * fh2x2(-5))*ckm2(6,5)
c
        sf(5,2,1,2)=dble(fh1x1(-2) * fh2x2( 2))*ckm2(6,2)
        sf(5,2,2,2)=dble(fh1x1(-3) * fh2x2( 3))*ckm2(6,3)
        sf(5,2,3,2)=dble(fh1x1(-5) * fh2x2( 5))*ckm2(6,5)
c
        sf(6,2,1,2)=dble(fh1x1( 2) * fh2x2(-2))*ckm2(6,2)
        sf(6,2,2,2)=dble(fh1x1( 3) * fh2x2(-3))*ckm2(6,3)
        sf(6,2,3,2)=dble(fh1x1( 5) * fh2x2(-5))*ckm2(6,5)
c
        sf(7,2,1,2)=dble(fh1x1(-2) * fh2x2(-2))*ckm2(6,2)
        sf(7,2,2,2)=dble(fh1x1(-3) * fh2x2(-3))*ckm2(6,3)
        sf(7,2,3,2)=dble(fh1x1(-5) * fh2x2(-5))*ckm2(6,5)
      endif
c jproc=3
      if(ittmin.eq.1)then
c t production
        sf(1,3,1,1)=dble(fh1x1( 2) * fh2x2( 0))*ckm2(6,2)
        sf(1,3,2,1)=dble(fh1x1( 3) * fh2x2( 0))*ckm2(6,3)
        sf(1,3,3,1)=dble(fh1x1( 5) * fh2x2( 0))*ckm2(6,5)
c
        sf(3,3,1,1)=dble(fh1x1( 0) * fh2x2( 2))*ckm2(6,2)
        sf(3,3,2,1)=dble(fh1x1( 0) * fh2x2( 3))*ckm2(6,3)
        sf(3,3,3,1)=dble(fh1x1( 0) * fh2x2( 5))*ckm2(6,5)
      endif
      if(ittmax.eq.2)then
c tbar production 
        sf(1,3,1,2)=dble(fh1x1(-2) * fh2x2( 0))*ckm2(6,2)
        sf(1,3,2,2)=dble(fh1x1(-3) * fh2x2( 0))*ckm2(6,3)
        sf(1,3,3,2)=dble(fh1x1(-5) * fh2x2( 0))*ckm2(6,5)
c
        sf(3,3,1,2)=dble(fh1x1( 0) * fh2x2(-2))*ckm2(6,2)
        sf(3,3,2,2)=dble(fh1x1( 0) * fh2x2(-3))*ckm2(6,3)
        sf(3,3,3,2)=dble(fh1x1( 0) * fh2x2(-5))*ckm2(6,5)
      endif
...
c
      ...
      elseif(ittintf.ne.0)then
        write(*,*)'Error in strfunwt: unknown ittintf',ittintf
      endif
c
      return
      end
...
c
c
      function sig5azw_wt(xx)
c Integrand function for H events
      implicit none
      real * 8 sig5azw_wt,xx
      real * 8 pi,tiny
      parameter (pi=3.14159265358979312D0)
     ...
     #  s,xmi2,xalfaem,rox,cth1,th2,cth2,tot5a_wt
c
...
c
      ...
      cth2 = cos(th2)
c
      sig5azw_wt = tot5a_wt(s,x,yi,cth1,cth2,xjac)
      return
      end


      function tot5a_wt(s,x,yi,cth1,cth2,xjac)
      implicit none
      real * 8 tot5a_wt,tot5as_wt,tot5az_wt,s,x,yi,cth1,cth2,
     #  xjac,tmp
      integer isubttype
      common/cisubttype/isubttype
c
      if(isubttype.eq.0)then
        tmp=tot5as_wt(s,x,yi,cth1,cth2,xjac)
      elseif(isubttype.eq.1)then
        tmp=tot5az_wt(s,x,yi,cth1,cth2,xjac)
      else
        write(*,*)'Fatal error in tot5a_wt:',isubttype
        stop
      endif
      tot5a_wt=tmp
      return
      ...
      function tot5as_wt(xs,xx,xyi,xcth1,xcth2,xjac)
c Implements standard subtraction
      implicit none
      real * 8 tot5as_wt,xs,xx,xyi,xcth1,xcth2,xjac
      real * 8 pi,pi2,hc2
      parameter (pi=3.14159265358979312D0)
      parameter (pi2 = pi*pi)
...
c sigma (pb) = 10^6 * sigma (mub)
      ...
      real * 8 rsf(7,3,24,2),vecsub(7)
      real*8 s,x,yi,cth1,cth2,sx,xii,ro,beta,rox,betax,xphsp_ns,
     #  xphsp_s,zgw2,x1,x2,x1t,x2t,tk,uk,q1q,q2q,zg2_nlo,zgmu2_nlo,
     #  xnorm,zg2_mc,zgmu2_mc,gfactsf,gfactcl,zhwfct,x1soft,x2soft,
     #  betafact,x1x2j,x1x2jac,ytmp,xfact,xtmp,xsum,dummy,xint,x1p,x2p
...
c
     ...
     #                tk,uk,q1q,q2q,xinv)
        zg2_nlo = zgmu2_nlo() 
        ipdfscale=1
        call strfunwt(x1,x2,sf)
C This corresponds to Eq. (4.37) in FKS
        xnorm = zgw2 * zg2_nlo**2 * xphsp_ns
        xnorm = xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
        do jproc=loproc,maproc
          if(jproc.ne.3)then
            call frealwtme(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,jproc,vecre)
          else
            call frealwt(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,jproc,vecre)
          endif
c Define the resonant antitop subtraction terms (remove the ttbar)
          call frealwtsub(jproc,xm12,xm22,s,x,yi,cth1,cth2,
     #                    tk,uk,q1q,q2q,x1,x2,x1p,x2p,vecsub) 
          call fluxrwgt(x1p,x2p,sf,rsf)
          do j=1,idrmax(jproc,jwt)
            do itype=1,jtypemax(jproc,j)
              do itt=ittmin,ittmax
                jp=idrmap(j,jproc,itt)
                vv(j,jproc,itype,itt)=vv(j,jproc,itype,itt)+
     #            sf(j,jproc,itype,itt)*xnorm*
     #            ( vecre(jp)-rsf(j,jproc,itype,itt)*vecsub(jp) )
              enddo
            enddo
          enddo
        enddo
...
c
        zg2_mc = zgmu2_mc()
        ipdfscale=2
        xnorm = zgw2 * zg2_mc**2 * xphsp_ns
        xnorm = xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
        do jproc=loproc,maproc
          call xmcsubtwt(jproc,x1,x2,xm12,xm22,s,x,yi,cth1,cth2,
...
c
     ...
     #                      tk,uk,q1q,q2q,xinv)
              ...
              ipdfscale=1
              call strfunwt(x1t,x2t,sf)
              xnorm = xfact * x1x2j * zgw2 * zg2_nlo**2 * xphsp_ns
              xnorm = xnorm * (1-gfsf(jproc))
              if(jproc.ne.3)then
                call frealwtme(s,x,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                         jproc,vecmccl)
              else
                call frealwt(s,x,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                       jproc,vecmccl)
              endif
              do j=1,idrmax(jproc,jwt)
                do itype=1,jtypemax(jproc,j)
...
c
     ...
     #                      tk,uk,q1q,q2q,xinv)
              ...
              ipdfscale=1
              call strfunwt(x1t,x2t,sf)
              xnorm = x1x2j * zgw2 * zg2_nlo**2 * xphsp_s 
              xnorm = xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
              xnorm = xnorm * (1-gfsf(jproc))
              if(jproc.ne.3)then
                call frealwtme(sx,xtmp,yi,cth2,tk,uk,q1q,q2q,xinv,
     #                         jproc,vecmcsf)
              else
                call frealwt(sx,xtmp,yi,cth2,tk,uk,q1q,q2q,xinv,
     #                       jproc,vecmcsf)
              endif
              do j=1,idrmax(jproc,jwt)
                do itype=1,jtypemax(jproc,j)
...
c
     ...
     #                        tk,uk,q1q,q2q,xinv)
                xnorm = xfact * x1x2j * zgw2 * zg2_nlo**2 * xphsp_s
                xnorm = - xnorm * (1-gfsf(jproc))
                if(jproc.ne.3)then
                  call frealwtme(sx,xtmp,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                           jproc,vecmcsc)
                else
                  call frealwt(sx,xtmp,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     #                         jproc,vecmcsc)
                endif
                do j=1,idrmax(jproc,jwt)
                  do itype=1,jtypemax(jproc,j)
...
C happens to be too large
      function tot5az_wt(xs,xx,xyi,xcth1,xcth2,xjac)
      implicit none
      real * 8 tot5az_wt,xs,xx,xyi,xcth1,xcth2,xjac
      return
      end


      function sig5bzw_wt(xx)
c Integrand function for S events
      implicit none
      real * 8 sig5bzw_wt,xx
      real * 8 pi,tiny
      parameter (pi=3.14159265358979312D0)
      parameter (tiny=1.d-5)
      dimension xx(6)
      ...
      real * 8 xjac,rohlim,zzz,x,ttt,th,yi,csi,rx,rohlimx,taumax,
     #  ximax0,ximin0,tmp,ymax,ymin,xxa1,xxa2,xxc,xxymax,xxymin,
     #  s,xmi2,xalfaem,rox,cth1,th2,cth2,tot5b_wt
c
c xx(1) --> tau, xx(2)-->ycm, xx(3) --> x, xx(4) --> y, xx(5) --> cth1,
c xx(6) --> cth2
c
...
c
      ...
      cth2 = cos(th2)
c
      sig5bzw_wt = tot5b_wt(s,x,yi,cth1,cth2,xjac)
      return
      end


      function tot5b_wt(s,x,yi,cth1,cth2,xjac)
      implicit none
      real * 8 tot5b_wt,tot5bs_wt,tot5bz_wt,s,x,yi,cth1,cth2,
     #  xjac,tmp
      integer isubttype
      common/cisubttype/isubttype
c
      if(isubttype.eq.0)then
        tmp=tot5bs_wt(s,x,yi,cth1,cth2,xjac)
      elseif(isubttype.eq.1)then
        tmp=tot5bz_wt(s,x,yi,cth1,cth2,xjac)
      else
        write(*,*)'Fatal error in tot5b_wt:',isubttype
        stop
      endif
      tot5b_wt=tmp
      return
      ...
      function tot5bs_wt(xs,xx,xyi,xcth1,xcth2,xjac)
      implicit none
      real * 8 tot5bs_wt,xs,xx,xyi,xcth1,xcth2,xjac
      real * 8 pi,pi2,hc2
      parameter (pi=3.14159265358979312D0)
      parameter (pi2 = pi*pi)
c GeV to microbarn conversion factor: sigma (mub) = hc2 * sigma (GeV^-2)
...
c sigma (pb) = 10^6 * sigma (mub)
      ...
      real * 8 gfsf(3),gfcl(3)
      real * 8 s,x,yi,cth1,cth2,sx,xii,ro,beta,rox,betax,xphsp_ns,
     #  xphsp_s,xphspb,zgw2,x1,x2,x1t,x2t,tk,uk,q1q,q2q,zg2_mc,
...
c The arguments of the pdf, see (A.43) in FW; x1, x2 are called z1, z2 there 
     ...
     #                  tk,uk,q1q,q2q,xinv)
          zg2_nlo = zgmu2_nlo()
          call strfunwt(x1t,x2t,sf)
          xnorm = x1x2j * zgw2 * zg2_nlo**2 * xphsp_ns
...
c and is therefore conveniently included here, via f2pr
          ...
          xnormc = x1x2j * zgw2 * zg2_nlo**2 * xphspb /(8*pi2 * deltai)
          xnormc = xnormc/xii 
          do jproc=loproc,maproc
            if(jproc.ne.3)then
              call frealwtme(s,x,ytmp,cth2,tk,uk,q1q,q2q,xinv,
     ...
     #                     jproc,vecre)
            ...
            call f2prwt(s,q2q,x,x,ytmp,xlmude,jproc,veccl)
            do j=1,idrmax(jproc,jwt)
              do itype=1,jtypemax(jproc,j)
                do itt=ittmin,ittmax
                  jp=idrmap(j,jproc,itt)
...
c
     ...
     #                     jproc,vecre)
            ...
            call f2prwt(s,q1q,x,x,ytmp,xlmude,jproc,veccl)
            do j=1,idrmax(jproc,jwt)
              do itype=1,jtypemax(jproc,j)
                do itt=ittmin,ittmax
                  jp=idrmap(j,jproc,itt)
...
c
     ...
     #                  tk,uk,q1q,q2q,xinv)
          ...
          xnorm = x1x2j * zgw2 * zg2_nlo**2 * xphsp_s 
          xnorm = - xnorm * 1.d0/xii*( 1/(1-yi) + 1/(1+yi) )
          xnormb = x1x2j * zgw2 * zg2_nlo * xphspb /(2*xicut)
          xnormsv = x1x2j * zgw2 * zg2_nlo**2 * xphspb / 
     #             (8*pi2 * 2*xicut)
          do jproc=loproc,maproc
            if(jproc.ne.3)then
              call frealwtme(sx,xtmp,yi,cth2,tk,uk,q1q,q2q,xinv,
     ...
     #                     jproc,vecre)
            ...
            call fbornwt(sx,q1q,jproc,vecbrn)
            call f2svwt(sx,q1q,jproc,vec2sv)
            do j=1,idrmax(jproc,jwt)
...
c
     ...
     #                    tk,uk,q1q,q2q,xinv)
            ...
            xnormc = x1x2j * zgw2 * zg2_nlo**2 * xphspb /
     #               (8*pi2 * deltai)
            xnormc = -xnormc/xii
            do jproc=loproc,maproc
              if(jproc.ne.3)then
     ...
     #                       jproc,vecre)
              ...
              call f2prwt(sx,q2q,x,xtmp,ytmp,xlmude,jproc,veccl)
              do j=1,idrmax(jproc,jwt)
...
c
     ...
     #                       jproc,vecre)
              ...
              call f2prwt(sx,q1q,x,xtmp,ytmp,xlmude,jproc,veccl)
              do j=1,idrmax(jproc,jwt)
                xintsvc(j,jproc)=xintsvc(j,jproc)+
     #                           xnorm*vecre(j)
...
C if the number of negative weight events for single top
C happens to be too large
      function tot5bz_wt(xs,xx,xyi,xcth1,xcth2,xjac)
      implicit none
      real * 8 tot5bz_wt,xs,xx,xyi,xcth1,xcth2,xjac
      tot5bz_wt=0.d0
      return
      end
c
...
c Determines colour connections
     ...
     # wg15,wg16,pr15,wg17,wg18,pr17,wg19,wg20,pr19,WQqqq_1,WQqgg_1,
     # WQqqq_1_nores,WQqgg_1_nores
      real*8 xmom_cm(10,4)
      common/cxmomcm/xmom_cm
      integer i0,jproc0,itype0,ich0,itt0
      common/cidproc/i0,jproc0,itype0,ich0,itt0
...
c
          ...
          if(jproc0.eq.2.and.i0.eq.5)then
            wg9=WQqqq_1(xm22,xm12,t13,t12,t23,t1p,tp3,t2p)
            wg10=WQqqq_1_nores(xm22,xm12,t13,t23,t12,tp3,t1p,t2p)
            pr9=wg9/(wg9+wg10)
            if(crnd.le.pr9)then
              iccode=9
            else
          ...
          elseif(jproc0.eq.2.and.i0.eq.6)then
            wg11=WQqqq_1(xm22,xm12,t23,t12,t13,t2p,tp3,t1p)
            wg12=WQqqq_1_nores(xm22,xm12,t23,t13,t12,tp3,t2p,t1p)
            pr11=wg11/(wg11+wg12)
          ...
          elseif(jproc0.eq.2.and.i0.eq.7)then
            wg13=WQqqq_1(xm22,xm12,t12,t13,t23,t1p,t2p,tp3)
            wg14=WQqqq_1(xm22,xm12,t12,t23,t13,t2p,t1p,tp3)
            pr13=wg13/(wg13+wg14)
          ...
          elseif(jproc0.eq.1.and.i0.eq.1)then
            wg15=WQqgg_1_nores(xm22,xm12,t13,t23,t12,tp3,t1p,t2p)
            wg16=WQqgg_1_nores(xm22,xm12,t23,t13,t12,tp3,t2p,t1p)
            pr15=wg15/(wg15+wg16)
          ...
          elseif(jproc0.eq.3.and.i0.eq.1)then
            wg17=WQqgg_1(xm22,xm12,t12,t13,t23,t1p,t2p,tp3)
            wg18=WQqgg_1(xm22,xm12,t13,t12,t23,t1p,tp3,t2p)
            pr17=wg17/(wg17+wg18)
          ...
          elseif(jproc0.eq.3.and.i0.eq.3)then
            wg19=WQqgg_1(xm22,xm12,t12,t23,t13,t2p,t1p,tp3)
            wg20=WQqgg_1(xm22,xm12,t23,t12,t13,t2p,tp3,t1p)
            pr19=wg19/(wg19+wg20)
...
c
      ...
      implicit none
      include 'stpcblks.h'
      integer i2b,i,j,il,in,ib,ii,it,iv
...
c partons, 3 is the outgoing FKS parton, 4 is the top or antitop, 5 is the W.
c When the top and W decay, 6=l, 7=nu, 8=b are the decay products of the top, 
c 9=l-, 10=nubar are the decay products of the W. Momentum conservation is 
c (1+2)-(3+4+5)=0 or (1+2)-(3+6+7+8+9+10)=0. In the following, 4+5 will 
...
c ip5=W when the top and W don't decay, and ip3=final-state light parton,
c (ip4,ip5,ip6)=top decay products, (ip7,ip8)=W decay products when the top 
c and W decay
      integer ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8
      common/ci1part/ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8
...
c Next deal with the lepton pair; W has momentum wvec2
     ...
     #                      xmom_prime(ii,3),xmom_prime(ii,4))
        enddo
c Hard W decay; adapted from VV production
        iv=5
        il=9
        in=10
        call fillvec(xmom_prime(iv,1),xmom_prime(iv,2),
     #               xmom_prime(iv,3),xmom_prime(iv,4),xtmp)
        xlepnorm=xmss(iv)**2-2*(xmss(il)**2+xmss(in)**2)+
     #           (xmss(il)**2-xmss(in)**2)**2/xmss(iv)**2
        xlepnorm=sqrt(xlepnorm)/2.d0
        do j=1,3
          xavg3(j)=0.d0
        enddo
     ...
     #      (1+xsign*(xmss(il)**2-xmss(in)**2)/xmss(iv)**2)
...
c
      if(ichkmom.eq.0)then
        if(idec.eq.0)then
          ...
          call checkwdec2(xmom_prime,5,9,10)
...
c
      if(ichmin.ne.3.or.ichmax.ne.3)then
        write(*,*)'This version of xout must be called for Wt channel'
        stop
      endif
      i0=0
      jproc0=0
     ...
     #                     i0,jproc0,itype0,ich0,itt0)
c The top and W decay. In such a case, determine the identities of the decay 
...
c Determine the identities of top and W decay products. Proceed assuming
c that the final state is (top,W-); charge conjugate at the end if the
c final state is actually (tbar,W+)
      implicit none
...
c wfrac12 is the fraction of decays W->e+mu/W->e+mu+all quarks 
c (hard W)
c wfrac123 is the fraction of decays W->e+mu+tau/W->e+mu+tau+all quarks
c (hard W)
      ...
      integer ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8
      common/ci1part/ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8
      integer ip1s,ip2s,ip3s,ip4s,ip5s,ip6s,ip7s,ip8s
      common/ci1parts/ip1s,ip2s,ip3s,ip4s,ip5s,ip6s,ip7s,ip8s
      integer i0,jproc0,itype0,ich0,itt0
      common/cidproc/i0,jproc0,itype0,ich0,itt0
c
...
c A (tbar,W+) final state was produced: charge-conjugate
      if(itt0.eq.2)then
        ip4=-ip4
        ip5=-ip5
        ip6=-ip6
...
c the exact real and Born lepton matrix elements
      implicit none
      real*8 pi,tolerance,bdredfact,vwl,awl,half
      parameter (pi=3.14159265358979312D0)
      parameter (tolerance=1.d-2)
c Divide the bound by bdredfact to compensate for peculiar behaviour of
c tW cross section. May become a process-dependent correction if need be.
c This is 1/0.85
      parameter (bdredfact=1.176d0)
c Vector and axial couplings of W to fermions
      parameter (vwl=1.d0)
      parameter (awl=1.d0)
     ...
     # e2,f2,phitq1,cthtq1,phitq2,cthtq2,phiwl,cthwl,o,xbwmass3,
     # rat1,qphsp,q1,tk,uk,q1q,q2q,xdec,xmadevwt_dc,unxdec,xsngltwt,
     # dmfactb1,dmfact1,phspfact1,dmfact2,phspfact2,xboundb,rat,
     # xmadsubwt_dc,xsnglwtsub,x1p,x2p,
     # xinv(5),xtq(4),xbq(4),xtl(4),xtn(4),xwb(4),xwl(4),xwn(4)
      integer iborn,iproj,icross,jjprc,jidr,icntuw,iqcntuw,iusesubt
c
      if(ich0.ne.3)then
...
c Set top and W masses to be used by put_on_shell. Generate them in this
c routine when off-shell effects will be implemented
      tq12=xm12
      q22=xm22
...
c the soft/collinear limits. We also set iproj=2 in such a case
      ...
      if( xevsign.lt.0.d0 .and. iborn.eq.1 .and.
     #    ( (jjprc.eq.1.and.jidr.eq.1) .or.
     #      (jjprc.eq.2.and.jidr.eq.1) .or.
     #      (jjprc.eq.2.and.jidr.eq.2) .or.
     #      (jjprc.eq.2.and.jidr.eq.5) .or.
     #      (jjprc.eq.2.and.jidr.eq.6) ) )then
        iusesubt=1
        if(ifuntype.ne.1)then
          write(*,*)'Error #5 in getspincorr',ifuntype,iborn,jjprc,jidr
          stop
        endif
      else
        iusesubt=0
...
c kinematics for decayed and undecayed matrix elements
     ...
     #              tk,uk,q1q,q2q,xinv)
      if(iusesubt.eq.0)then
        call gentopdmom(xmt,q1,cthtq1,phitq1,cthtq2,phitq2,
     #                  xtq,xbq,xtl,xtn,1)
        if(ichkmom.eq.0)call checktdec1(xmt,xtq,xbq,xtl,xtn,1)
        call genwdmom(xmw,cthwl,phiwl,xwb,xwl,xwn,2)
        if(ichkmom.eq.0)call checkwdec1(xmw,xwb,xwl,xwn,2)
        if(ichkmom.eq.0)call checkmom(xmom_cm,ps,0.d0,10,1)
        xdec=xmadevwt_dc(iborn,jjprc,jidr,ps,tk,uk,xmom_cm)
        unxdec=xsngltwt(iborn,jjprc,jidr,xm12,xm22,ps,xtmp,yitmp,
     #                  pcth1,pcth2,tk,uk,q1q,q2q,xinv)
      elseif(iusesubt.eq.1)then
c The entries corresponding to x1 and x2 are set to 1/2 here, since
c they are used only for flux reweighting, which is not done in the
c computation of spin correlations
        xdec=xmadsubwt_dc(iborn,jjprc,jidr,xm12,xm22,ps,xtmp,yitmp,
     #                    pcth1,pcth2,tk,uk,q1q,q2q,half,half,x1p,x2p,
     #                    q1,cthtq1,phitq1,cthtq2,phitq2,cthwl,phiwl)
        unxdec=xsnglwtsub(iborn,jjprc,jidr,xm12,xm22,ps,xtmp,yitmp,
     #                    pcth1,pcth2,tk,uk,q1q,q2q,half,half,x1p,x2p)
      else
        write(*,*)'Error #8 in getspincorr',iusesubt
        stop
      endif
...
c e^4 -> gw^4; single-top cross sections in mcatnlo_stxsec factorize gw^4
      ...
      dmfact2=xmw2*(vwl+awl)**2/(4*sthw2)
c e^2 -> gw^2
      dmfact2=dmfact2*sthw2
      phspfact2=1.d0/(xm012*ga1**2)
      xboundb=dmfactb1*dmfact1*phspfact1*dmfact2*phspfact2
...
c
 ...
 900  format(4(1x,d14.8))
      ...
      function xsngltwt(iborn,jproc,idr,xm12,xm22,s,x,yi,cth1,cth2,
     #                tk,uk,q1q,q2q,xinv)
...
c For Born matrix elements, q1q is t (consistently with invar_in)
      implicit none
      real*8 xsngltwt,xm12,xm22,s,x,yi,cth1,cth2,tk,uk,q1q,q2q,xinv(5)
      integer iborn,jproc,idr
      real*8 xmatin(7)
c
      if(iborn.eq.0)then
        call fbornwt(s,q1q,jproc,xmatin)
      else
        if(jproc.ne.3)then
          call frealwtme(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,jproc,xmatin)
        else
          call frealwt(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,jproc,xmatin)
        endif
      endif
      xsngltwt=xmatin(idr)
      return
      ...
      function xsnglwtsub(iborn,jproc,idr,xm12,xm22,s,x,yi,cth1,cth2,
     #                    tk,uk,q1q,q2q,x1,x2,x1p,x2p)
c Wrapper for the undecayed matrix elements of the original code.
c For Born matrix elements, q1q is t (consistently with invar_in)
      implicit none
      real*8 xsnglwtsub,xm12,xm22,s,x,yi,cth1,cth2,tk,uk,q1q,q2q,
     # x1,x2,x1p,x2p
      integer iborn,jproc,idr
      real*8 xmatin(7)
c
      if(iborn.eq.0)then
        write(6,*)'Error #1 in xsnglwtsub'
        stop
      else
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.2.and.
     #       (idr.ne.1.and.idr.ne.2.and.idr.ne.5.and.idr.ne.6)) .or.
     #      jproc.eq.3 )then
          write(6,*)'Error #2 in xsnglwtsub:',jproc,idr
          stop
        endif
        call frealwtsub(jproc,xm12,xm22,s,x,yi,cth1,cth2,
     #                  tk,uk,q1q,q2q,x1,x2,x1p,x2p,xmatin)
      endif
      xsnglwtsub=xmatin(idr)
      return
      end
...
c
      ...
      implicit none
      include 'stpcblks.h'
      real * 8 pi,zero,one,aem,cthw2,sthw2,xmw,gaw,xmt,twidth
      real * 8 ruckm,rcckm,rtckm,rducckm,rsucckm,rbucckm
      parameter (pi=3.14159265358979312D0)
      parameter (one=1.d0)
      parameter (zero=0.d0)
...
c into account; with the following entries, it holds better than 0.1%
      ...
      do i=1,6
        ...
        enddo
      enddo
c Combinations used in strfunwt; need them also for unweighting
...
c equal to one, and use the actual values in the main code
      call setmepar(xmw,gaw,zero,zero,
...
c s- and t-channels
      ...
      idrlimcp(2,3,6)=3
      idrlimcp(2,3,8)=4
c Wt-channel, gg initial state
      idrlimcp(3,1,1)=1
      idrlimcm(3,1,1)=3
c Wt-channel, qq initial state
      idrlimcm(3,2,3)=1
      idrlimcp(3,2,4)=3
      idrlimcm(3,2,5)=1
      idrlimcp(3,2,6)=3
...
c for Wt-channel, see the routine strfunwt
      jtypemax(1,1)=1
      jtypemax(2,1)=4
      jtypemax(2,2)=4
      jtypemax(2,3)=24
...
c collinear y=1 splitting for the Wt channel. Set to zero if unused or
...
c this quantity is relevant to shower variables and scale 
      ...
      ialwsplit(3,3,2)=1
      ialwsplit(3,3,4)=1
c ie0wt(jproc,idr,ileg,ie0sq)=ie0sq if the corresponding combination in
...
c Process: iprdct=2000+IT   s+t+Wt channels
c                 2010+IT   s channel      
c                 2020+IT   t channel      
c                 2030+IT   Wt channel      
c with
...
c Values from PDG 2003
      ...
      integer inonbtop
      common/cinonbtop/inonbtop
c Parton identities: if top and W don't decay, 4 is top, 5 is W. If top
c and W decay, (4,5,6) are the decay products of the top, (7,8) those of
c the W. PDG conventions are used
      integer ip1s,ip2s,ip3s,ip4s,ip5s,ip6s,ip7s,ip8s
      common/ci1parts/ip1s,ip2s,ip3s,ip4s,ip5s,ip6s,ip7s,ip8s
...
c event-by-event basis if the event is (tbar,W+) (see getpdecids)
      ...
      elseif(il2hw.eq.0.or.(il2hw.ge.4.and.il2hw.le.6))then
        ...
        write(*,*)'Error #2 in setpardec: inconsistent entries',il2hw
        stop
      endif
...
c hard W; no kinematics and CKM correction factors here
      brrv2msb=1.d0
      if(il2hw.eq.0)then
        brrv2msb=(3*xbrrwlep+2*xbrrwhad) * brrv2msb
      elseif(il2hw.ge.1.and.il2hw.le.3)then
        brrv2msb=xbrrwlep * brrv2msb
      elseif(il2hw.eq.4)then
        brrv2msb=2*xbrrwlep * brrv2msb
      elseif(il2hw.eq.5)then
        brrv2msb=2*xbrrwhad * brrv2msb
      elseif(il2hw.eq.6)then
        brrv2msb=(2*xbrrwlep+2*xbrrwhad) * brrv2msb
      endif
c wfrac12 is the fraction of decays W->e+mu/W->e+mu+all quarks
      wfrac12=2*xbrrwlep/(2*xbrrwlep+2*xbrrwhad)
c wfrac123 is the fraction of decays W->e+mu+tau/W->e+mu+tau+all quarks
      wfrac123=3*xbrrwlep/(3*xbrrwlep+2*xbrrwhad)
c
...
c
 ...
 998  write(*,*)'format error'
      ...
      write(77,902)ip1,ip2,ip3,ip4,ip5,ip6,ip7,ip8
...
c partons, 3 is the outgoing parton, 4 is t, 5 is W. When the top and W
c decay, 6=l+, 7=nu, 8=b are the decay products of the top, 9=l-, 10=nubar
c are the decay products of the W. Momentum conservation is 
c (1+2)-(3+4+5)=0 or (1+2)-(3+6+7+8+9+10)=0
      real*8 xmom_lb(10,4)
      common/cxmomlb/xmom_lb
      integer iwgtnorm
      common/ciwgtnorm/iwgtnorm
...
c for the first time
     ...
     #                  ((xfact*xmom_lb(i,j),j=1,4),i=6,10)
      elseif(idec.eq.1)then
...
c
c
      subroutine xmcsubtwt(jproc,z1,z2,xm12,xm22,s,x,yi,cth1,cth2,
     #                     gfactsf,gfactcl,flagmc,lzone,z,xmatmc)
c Computes the MC counterterms for single-top production, Wt channel. 
...
c For all the array outputs, (7,3,6) correspond to (idr,ileg,ie0sq)
     ...
     # q1q,q2q,e0sq,de0sqdx,de0sqdc,xz,zherwwt,xxi,xiherwwt,
     # xalsfwt,xbesfwt,xalclwt,xbeclwt,xalazi,xbeazi,
     # gfunsoftwt,gfactazi,gfunazi,sbar,tbar,tmp1,tmp,
     # cfact,sth1,cthg,beta,tbeta10,ap_kern,qin_kern,xfact,ap,
     # xjac,xjac_xiztoxywt,qk,dcosdcth1,
     # xinv(5),ae0sq(6),ade0sqdx(6),ade0sqdc(6),
     # x2to2(3,6),dx2to2dx(3,6),dx2to2dc(3,6),
     # xmcz(3,6),xmcxi(3,6),xmce0sq(3,6),
     # xborn(7),xazi(7)
      integer ilegmax,ileg,ie0sq,icllazi,i,icode,iborn
      logical flagxs(3),flxsec(3,6)
      real*8 xicut,deltai,deltao
      common/parsub/xicut,deltai,deltao
      real*8 alsfwt(3),besfwt(3)
      common/cgfunsfwt/alsfwt,besfwt
      real*8 alclwt(3),beclwt(3)
      common/cgfunclwt/alclwt,beclwt
...
c corresponding ones for ileg=1, so we skip the generation in such a case
     ...
     #      ss,tk,uk,q1q,q2q)
          call gete0sq(jwt,ileg,z1,z2,xm12,xm22,ss,x,yi,cth1,cth2,
...
c Now get z and xi for possible splittings
     ...
     #    ss,tk,uk,q1q,q2q)
        do ie0sq=1,6
          if(ialwsplit(jproc,ileg,ie0sq).eq.1.and.flagxs(ileg))then
            ...
            xz=zherwwt(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                 tk,uk,q1q,q2q)
            xxi=xiherwwt(ileg,e0sq,xm12,xm22,ss,x,yi,cth1,cth2,
     #                   tk,uk,q1q,q2q)
            if(ileg.le.2)then
     ...
     #            xz**2.ge.xxi )then
              ...
              if( xz.ge.0.d0 .and. xxi.ge.0.d0. and.
     #            (e0sq*xxi*xz**2).ge.xm12.and.xxi.le.1.d0 )then
                flxsec(ileg,ie0sq)=.true.
                if(.not.flagmc)flagmc=.true.
              else
                flxsec(ileg,ie0sq)=.false.
...
c to compute the ME part of the MC subtraction term
      xalsfwt=alsfwt(jproc)
      xbesfwt=besfwt(jproc)
      xalclwt=alclwt(jproc)
      xbeclwt=beclwt(jproc)
      xalazi=alazi(jproc)
      xbeazi=beazi(jproc)
      gfactsf=gfunsoftwt(x,ss,xm12,xm22,xalsfwt,xbesfwt)
      gfactcl=-1.d8
...
c associate it to the first non-zero ie0sq contribution
     ...
     #      ss,tk,uk,q1q,q2q)
          ...
          call fbornwt(sbar,tbar,ithree,xborn)
          if( gfactazi.ne.0.d0 .and. ileg.le.2 .and.
     #        (jproc.eq.2.or.jproc.eq.3) )then
            icllazi=1-2*(ileg-1)
            call faziwt(sbar,tbar,cth2,jproc,icllazi,xazi)
          endif
          do i=1,idrmax(jproc,jwt)
            tmp1=0.d0
            do ie0sq=1,6
              if( flxsec(ileg,ie0sq) .and.
     #            ie0wt(jproc,i,ileg,ie0sq).eq.ie0sq )then
                ...
                if(1-x.lt.tiny.and.ileg.le.2)then
                  cfact=0.d0
                  if(jproc.eq.3)then
                    if( (i.eq.1.and.ileg.eq.1) .or.
     #                (i.eq.3.and.ileg.eq.2) )then
                      cfact=vcf
                    elseif( (i.eq.1.and.ileg.eq.2) .or.
     #                    (i.eq.3.and.ileg.eq.1) )then
                      cfact=vca
                    endif
                  endif
                  if(cfact.ne.0.d0)then
                    if(ileg.eq.1)then
                      tmp=64*e0sq*(1+yi)/(s*(s*(1-yi)+4*e0sq*(1+yi)))
                    elseif(ileg.eq.2)then
                      tmp=64*e0sq*(1-yi)/(s*(s*(1+yi)+4*e0sq*(1-yi)))
                    endif
                  endif
                ...
                elseif(1-x.lt.tiny.and.ileg.eq.3.and.jproc.eq.3)then
                  ...
                  beta=sqrt(1-2*(xm12+xm22)/ss+(xm12-xm22)**2/ss**2)
                  tbeta10=sqrt(1-xm12/e0sq)
                  tmp=e0sq*(ss*(1+beta)+xm12-xm22)*tbeta10*(1+tbeta10)/
     #              ( (ss*(1-beta*cthg)+xm12-xm22)*(
     #                  ss*(ss*(1+beta)*(1-beta*cthg)+
     #                  4*(1+cthg)*e0sq*tbeta10*(1+tbeta10))+
     #              (xm12-xm22)*(ss*(2+beta*(1-cthg))+xm12-xm22) ) )
                  tmp=32*vcf*(1-yi**2)*abs(tmp)
     ...
     #              tmp1=8*qin_kern(x,icode)/ss
                  ...
                  de0sqdx=dx2to2dx(ileg,ie0sq)
                  de0sqdc=dx2to2dc(ileg,ie0sq)
                  xjac=xjac_xiztoxywt(ileg,e0sq,de0sqdx,de0sqdc,
     ...
     #                ileg.le.2.and.(jproc.eq.2.or.jproc.eq.3) )then
                ...
                if(ileg.eq.1)then
                  iborn=idrlimcp(jwt,jproc,i)
                elseif(ileg.eq.2)then
                  iborn=idrlimcm(jwt,jproc,i)
                else
                  iborn=i
                endif
...
c
      if(besf.lt.0.d0)then
        ...
        elseif(isubttype.eq.1)then
          ...
          write(*,*)'Fatal error #1 in gfunsoftwt',isubttype
          stop
        endif
      endif
      ...
      if(tt.gt.1.d0)then
        write(6,*)'Fatal error #2 in gfunsoftwt',x
        stop
      endif
      tmp=1.d0
...
c computed for single-top production, Wt channel. Legs 1,2 are identical
...
c  w2  = q1q-q2q-uk
c
      function zherwwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                 xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable z; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 zherwwt,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,zherw,w1,w2,beta,tbeta10,sth1,cthg,beta2,
     # tbeta1,eps2,zeta1
      parameter (tiny=1.d-4)
...
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        zherwwt=zherw(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        ...
        if(1-x.lt.tiny)then
          ...
          cthg=yi*cth1+sqrt(1-yi**2)*cth2*sth1
          zherwwt=1+(x-1)*(
     #      s*(1-beta*cthg)/(4*e0sq*(1+tbeta10))+
     #      (1+cthg)*tbeta10/(1+beta)+
     #      (xm12-xm22)*( 1/(4*e0sq*(1+tbeta10))-
     #           (1+cthg)*tbeta10/((1+beta)*(xm12-xm22+s*(1+beta))) ) )
        else
     ...
     #          ( (s-w1)*beta2*(2*s-(s-w1)*eps2+(s-w1)*beta2) )
          zherwwt=1-tbeta1*zeta1-w1/(2*(1+tbeta1)*e0sq)
        endif
      else
        write(6,*)'zherwwt: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xiherwwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                  xtk,xuk,xq1q,xq2q)
c Returns Herwig shower variable xi; xm12 and xm22 are the top and W 
c masses squared
      implicit none
      real * 8 xiherwwt,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     # xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,xiherw,w1,beta,tbeta10,sth1,cthg,z,zherwwt
      parameter (tiny=1.d-3)
      parameter (dummy=-1.d3)
c
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        xiherwwt=xiherw(ileg,e0sq,xm12,s,x,yi,dummy,xtk,xuk,xq1q,xq2q)
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        w1=-xq1q+xq2q-xtk
        if(e0sq.le.(w1+xm12))then
        ...
        if(1-x.lt.tiny)then
          ...
          sth1=sqrt(1-cth1**2)
          cthg=yi*cth1+sqrt(1-yi**2)*cth2*sth1
          xiherwwt=( s**2*(1+beta)*(1-beta*cthg)*(1+tbeta10) +
     #      (xm12-xm22)*(xm12-xm22+s*(2+beta*(1-cthg)))*(1+tbeta10) )/
     #      (  (1+beta)*(1-beta*cthg)*s**2+
     #         4*s*(1+cthg)*e0sq*tbeta10*(1+tbeta10)+
     #         (xm12-xm22)*(xm12-xm22+s*(2+beta*(1-cthg))) )
        else
          z=zherwwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #              xtk,xuk,xq1q,xq2q)
          if(z.gt.0.d0)then
            xiherwwt=w1/(2*z*(1-z)*e0sq)
          else
            xiherwwt=-1.d0
          endif
        endif
      else
        write(6,*)'xiherwwt: unknown parton number'
        stop
      endif
      return
      end
      ...
      function xjac_xiztoxywt(ileg,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
...
c variables, and x and c are FKS variables
      implicit none 
      real * 8 xjac_xiztoxywt,e0sq,de0sqdx,de0sqdc,xm12,xm22,s,
     # x,yi,cth1,cth2,xtk,xuk,xq1q,xq2q
      integer ileg
      real * 8 tiny,dummy,tmp,xjac_xiztoxy,w1,w2,sth1,beta,tbeta10,
     # cthg,zmo,z,zherwwt,xi,xiherwwt,cpsip,spsip,beta2,tbeta1,
     # betax,eps2,zeta1,dw1dx,dw1dc,dw2dx,dw2dc,dzeta1dx,dzeta1dc,
     # cf1,cf2,cf3,cf4
      parameter (tiny=1.d-4)
      parameter (dummy=-1.d3)
c
...
c incoming partons #1 (left) and #2 (right)
      if(ileg.eq.1.or.ileg.eq.2)then
        tmp=xjac_xiztoxy(ileg,e0sq,de0sqdx,de0sqdc,xm12,s,
...
c outgoing parton #3 (top)
      elseif(ileg.eq.3)then
        ...
        if(1-x.lt.tiny)then
          ...
          tbeta10=sqrt(1-xm12/e0sq)
          cthg=yi*cth1+sqrt(1-yi**2)*cth2*sth1
          zmo=s*(1-beta*cthg)/(4*e0sq*(1+tbeta10))+
     #        (1+cthg)*tbeta10/(1+beta)+
     #      (xm12-xm22)*( 1/(4*e0sq*(1+tbeta10))-
     #           (1+cthg)*tbeta10/((1+beta)*(xm12-xm22+s*(1+beta))) )
          tmp=s*tbeta10/(4*e0sq*zmo)
        else
          z=zherwwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #              xtk,xuk,xq1q,xq2q)
          if(z.gt.0.d0)then
            xi=xiherwwt(ileg,e0sq,xm12,xm22,s,x,yi,cth1,cth2,
     #                  xtk,xuk,xq1q,xq2q)
     ...
     #             (betax*s**2*x**3)-(xm12-xm22)/(2*x**2)
            ...
            dw2dx=-dw1dx-s
            dw2dc=-dw1dc
            dzeta1dx=dw2dx/(beta2*(s-w1))+
     #        dw1dx*zeta1*( 1/(s-w1)+
     #         (4*xm22*s+eps2*(xm12-xm22)*(s-w1))/(beta2**2*(s-w1)**3)+
     #         ((s-w1-xm12+xm22)-beta2*(s-w1))/
     #         (beta2*(s-w1)*(2*s+(beta2-eps2)*(s-w1))) )+
     #        dw1dx*( (w1+w2)*(beta2-eps2)*(1-beta2-eps2)/beta2+
     #                (s-w1)*(beta2-eps2)-4*xm22*s*(w1+w2)/
     #                (beta2*(s-w1)**2) )/
     #              (beta2*(s-w1)*(2*s+(beta2-eps2)*(s-w1)))
            dzeta1dc=dw2dc/(beta2*(s-w1))+
     #        dw1dc*zeta1*( 1/(s-w1)+
     #         (4*xm22*s+eps2*(xm12-xm22)*(s-w1))/(beta2**2*(s-w1)**3)+
     #         ((s-w1-xm12+xm22)-beta2*(s-w1))/
     #         (beta2*(s-w1)*(2*s+(beta2-eps2)*(s-w1))) )+
     #        dw1dc*( (w1+w2)*(beta2-eps2)*(1-beta2-eps2)/beta2+
     #                (s-w1)*(beta2-eps2)-4*xm22*s*(w1+w2)/
     #                (beta2*(s-w1)**2) )/
     #              (beta2*(s-w1)*(2*s+(beta2-eps2)*(s-w1)))
            cf1=(dw1dc*dw2dx-dw1dx*dw2dc)/(beta2*(s-w1))
            cf2=(de0sqdx*dw1dc-de0sqdc*dw1dx)*(w1+xm12)/
     #          (2*e0sq**2*tbeta1)
            cf3=de0sqdc*dzeta1dx-de0sqdx*dzeta1dc
            cf4=(de0sqdx*dw1dc-de0sqdc*dw1dx)/(2*e0sq*tbeta1)
            tmp=-( (1+tbeta1)*cf1+
     #             (1-(1+xi)*z+xi*z**2+tbeta1**2*zeta1)/tbeta1**2*cf2 -
     #             w1/e0sq*( (1+tbeta1)*cf3 +
     #             (1-(1+xi)*z+xi*z**2+tbeta1**2*zeta1)/tbeta1**2*cf4 ) 
     #            )*tbeta1/( 2*e0sq*(1+tbeta1)*z*(1-z) )
            tmp=tmp*4*x/(1+yi+x*(1-yi))**2
          endif
        endif
      else
        write(6,*)'xjac_xiztoxywt: unknown parton number'
        stop
      endif
      xjac_xiztoxywt=abs(tmp)
      return 
      end 
...
c In the case of Wt channel (ich=3) the emission from leg 3 is treated
c similarly to what done in the case of QQbar production, since we use 
c an initial-state parametrization. In order to treat Wt channel, xm22
...
c
     ...
     #                   (beta2*(s-w1)**2) )
      ...
      elseif(ileg.eq.4)then
        if(ich.eq.3)then
          write(*,*)'gete0sq: Wt channel with leg 4'
          stop
        endif
        sbar=s
        dsbardx=0.d0
...
c
c
c Here include z,xi,xjac taken from the s- and t-channel
c
c
