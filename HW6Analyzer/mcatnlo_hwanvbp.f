...
c
      ...
      do j=1,2
      k=(j-1)*50
      call mbook(k+ 1,'V1 pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+ 2,'V1 log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+ 3,'V2 pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+ 4,'V2 log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+ 5,'y_V1'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 6,'y_V2'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 7,'y_V1-y_V2'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 8,'eta_V1-eta_V2'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 9,'VV inv m'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+10,'y_VV'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+11,'VV pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+12,'VV log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+13,'VV azimt'//cc(j),pi/20.e0,0.e0,pi)
      call mbook(k+14,'VV log[pi-azimt]'//cc(j),0.05e0,-4.e0,0.1e0)
      call mbook(k+15,'M_V1'//cc(j),sngl(bin1),xm1i,xm1s)
      call mbook(k+16,'M_V2'//cc(j),sngl(bin2),xm2i,xm2s)
c
      if(idec.eq.0)then
      k=(j-1)*50+16
      call mbook(k+ 1,'l1 pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+ 2,'l1 log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+ 3,'l2 pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+ 4,'l2 log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+ 5,'y_l1'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 6,'y_l2'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 7,'y_l1-y_l2'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+ 8,'ll inv m'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+ 9,'y_ll'//cc(j),0.2e0,-4.e0,4.e0)
      call mbook(k+10,'ll pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+11,'ll log[pt]'//cc(j),0.05e0,-0.5e0,3.e0)
      call mbook(k+12,'ll azimt'//cc(j),pi/20.e0,0.e0,pi)
      endif
c
      enddo
c
...
C----------------------------------------------------------------------
      ...
      COMMON/VVLIN/IVLEP1,IVLEP2
      OPEN(UNIT=99,FILE='HERVB.TOP',STATUS='UNKNOWN')
...
C
      if(ivlep1.gt.6)then
        if(ivlep2.le.6)call hwwarn('HWAEND',501)
        idec=1
      else
        if(ivlep2.gt.6)call hwwarn('HWAEND',501)
        idec=0
      endif
c
      do j=1,2
      k=(j-1)*50
      call multitop(100+k+ 1,99,2,3,'V1 pt',' ','LOG')
      call multitop(100+k+ 2,99,2,3,'V1 log[pt]',' ','LOG')
      call multitop(100+k+ 3,99,2,3,'V2 pt',' ','LOG')
      call multitop(100+k+ 4,99,2,3,'V2 log[pt]',' ','LOG')
      call multitop(100+k+ 5,99,2,3,'V1 y',' ','LOG')
      call multitop(100+k+ 6,99,2,3,'V2 y',' ','LOG')
      call multitop(100+k+ 7,99,2,3,'y_V1-y_V2',' ','LOG')
      call multitop(100+k+ 8,99,2,3,'eta_V1-eta_V2',' ','LOG')
      call multitop(100+k+ 9,99,2,3,'VV inv m',' ','LOG')
      call multitop(100+k+10,99,2,3,'y_VV',' ','LOG')
      call multitop(100+k+11,99,2,3,'VV pt',' ','LOG')
      call multitop(100+k+12,99,2,3,'VV log[pt]',' ','LOG')
      call multitop(100+k+13,99,2,3,'VV azimt',' ','LOG')
      call multitop(100+k+14,99,2,3,'VV azimt',' ','LOG')
      call multitop(100+k+15,99,2,3,'m_V1',' ','LOG')
      call multitop(100+k+16,99,2,3,'m_V2',' ','LOG')
      enddo
c
      if(idec.eq.0)then
      do j=1,2
      k=(j-1)*50+16
      call multitop(100+k+ 1,99,2,3,'l1 pt',' ','LOG')
      call multitop(100+k+ 2,99,2,3,'l1 log[pt]',' ','LOG')
      call multitop(100+k+ 3,99,2,3,'l2 pt',' ','LOG')
      call multitop(100+k+ 4,99,2,3,'l2 log[pt]',' ','LOG')
      call multitop(100+k+ 5,99,2,3,'l1 y',' ','LOG')
      call multitop(100+k+ 6,99,2,3,'l2 y',' ','LOG')
      call multitop(100+k+ 7,99,2,3,'y_l1-y_l2',' ','LOG')
      call multitop(100+k+ 8,99,2,3,'ll inv m',' ','LOG')
      call multitop(100+k+ 9,99,2,3,'y_ll',' ','LOG')
      call multitop(100+k+10,99,2,3,'ll pt',' ','LOG')
      call multitop(100+k+11,99,2,3,'ll log[pt]',' ','LOG')
      call multitop(100+k+12,99,2,3,'ll azimt',' ','LOG')
      enddo
      endif
c
      CLOSE(99)
...
c
      kk=0
      call mfill(kk+1,sngl(ptv1),sngl(WWW0))
      if(ptv1.gt.0.d0)call mfill(kk+2,sngl(log10(ptv1)),sngl(WWW0))
      call mfill(kk+3,sngl(ptv2),sngl(WWW0))
      if(ptv2.gt.0.d0)call mfill(kk+4,sngl(log10(ptv2)),sngl(WWW0))
      call mfill(kk+5,sngl(yv1),sngl(WWW0))
      call mfill(kk+6,sngl(yv2),sngl(WWW0))
      call mfill(kk+7,sngl(yv1-yv2),sngl(WWW0))
      call mfill(kk+8,sngl(etav1-etav2),sngl(WWW0))
      call mfill(kk+9,sngl(xmvv),sngl(WWW0))
      call mfill(kk+10,sngl(yvv),sngl(WWW0))
      call mfill(kk+11,sngl(ptvv),sngl(WWW0))
      if(ptvv.gt.0.d0)call mfill(kk+12,sngl(log10(ptvv)),sngl(WWW0))
      call mfill(kk+13,sngl(dphivv),sngl(WWW0))
      if(xldphivv.gt.0.d0)
     #  call mfill(kk+14,sngl(log10(xldphivv)),sngl(WWW0))
      call mfill(kk+15,sngl(xmv1),sngl(WWW0))
      call mfill(kk+16,sngl(xmv2),sngl(WWW0))
c
      if(idec.eq.0)then
        kk=16
        call mfill(kk+1,sngl(pte1),sngl(WWW0))
        if(pte1.gt.0.d0)call mfill(kk+2,sngl(log10(pte1)),sngl(WWW0))
        call mfill(kk+3,sngl(pte2),sngl(WWW0))
        if(pte2.gt.0.d0)call mfill(kk+4,sngl(log10(pte2)),sngl(WWW0))
        call mfill(kk+5,sngl(ye1),sngl(WWW0))
        call mfill(kk+6,sngl(ye2),sngl(WWW0))
        call mfill(kk+7,sngl(ye1-ye2),sngl(WWW0))
        call mfill(kk+8,sngl(xmee),sngl(WWW0))
        call mfill(kk+9,sngl(yee),sngl(WWW0))
        call mfill(kk+10,sngl(ptee),sngl(WWW0))
        if(ptee.gt.0.d0)call mfill(kk+11,sngl(log10(ptee)),sngl(WWW0))
        call mfill(kk+12,sngl(dphiee),sngl(WWW0))
      endif
c
      kk=50
      if(abs(yv1).lt.2.d0)then
        call mfill(kk+1,sngl(ptv1),sngl(WWW0))
        if(ptv1.gt.0.d0)call mfill(kk+2,sngl(log10(ptv1)),sngl(WWW0))
        if(ptv1.gt.20.d0)call mfill(kk+15,sngl(xmv1),sngl(WWW0))
      endif
      if(abs(yv2).lt.2.d0)then
        call mfill(kk+3,sngl(ptv2),sngl(WWW0))
        if(ptv2.gt.0.d0)call mfill(kk+4,sngl(log10(ptv2)),sngl(WWW0))
        if(ptv2.gt.20.d0)call mfill(kk+16,sngl(xmv2),sngl(WWW0))
      endif
      if(ptv1.gt.20.d0)call mfill(kk+5,sngl(yv1),sngl(WWW0))
      if(ptv2.gt.20.d0)call mfill(kk+6,sngl(yv2),sngl(WWW0))
...
c
      if(idec.eq.0)then
        ...
        if(pte1.gt.20.d0)call mfill(kk+5,sngl(ye1),sngl(WWW0))
        if(pte2.gt.20.d0)call mfill(kk+6,sngl(ye2),sngl(WWW0))
