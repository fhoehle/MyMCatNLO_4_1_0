...
c
      ...
      call mbook(k+ 6,'mV'//cc(j),sngl(binv),xmvi,xmvs)
c
      call mbook(k+ 7,'H pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+ 8,'H pt'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+ 9,'H log[pt]'//cc(j),0.05e0,0.1e0,5.e0)
      call mbook(k+10,'H y'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+11,'H eta'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+12,'mH'//cc(j),sngl(binh),xmhi,xmhs)
c
      call mbook(k+13,'VH pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+14,'VH pt'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+15,'VH log[pt]'//cc(j),0.05e0,0.1e0,5.e0)
      call mbook(k+16,'VH y'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+17,'VH eta'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+18,'mVH'//cc(j),sngl(bini),xmii,xmis)
      call mbook(k+19,'VH azimt'//cc(j),pi/20.e0,0.e0,pi)
      call mbook(k+20,'VH log[pi-azimt]'//cc(j),0.05e0,-4.e0,0.1e0)
      call mbook(k+21,'VH delta eta'//cc(j),0.2e0,-9.e0,9.e0)
c
      enddo
c
      if(idec.eq.0)then
      do j=1,2
      k=(j-1)*50
      call mbook(k+25+ 1,'l pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+25+ 2,'l pt'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+25+ 3,'l log[pt]'//cc(j),0.05e0,0.1e0,5.e0)
      call mbook(k+25+ 4,'l eta'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+25+ 5,'lb pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+25+ 6,'lb pt'//cc(j),10.e0,0.e0,1000.e0)
      call mbook(k+25+ 7,'lb log[pt]'//cc(j),0.05e0,0.1e0,5.e0)
      call mbook(k+25+ 8,'lb eta'//cc(j),0.2e0,-9.e0,9.e0)
c                       
      call mbook(k+25+ 9,'llb delta eta'//cc(j),0.2e0,-9.e0,9.e0)
      call mbook(k+25+10,'llb azimt'//cc(j),pi/20.e0,0.e0,pi)
      call mbook(k+25+11,'llb log[pi-azimt]'//cc(j),0.05e0,-4.e0,0.1e0)
      call mbook(k+25+12,'llb inv m'//cc(j),sngl(binv),xmvi,xmvs)
      call mbook(k+25+13,'llb pt'//cc(j),2.e0,0.e0,200.e0)
      call mbook(k+25+14,'llb log[pt]'//cc(j),0.05e0,0.1e0,5.e0)
c
      enddo
      endif
 999  END
...
C----------------------------------------------------------------------
      INCLUDE 'HERWIG65.INC'
      REAL*8 XNORM
      INTEGER I,J,K,IDEC,IVHVEC,IVHLEP
      COMMON/VHLIN/IVHVEC,IVHLEP
C
      OPEN(UNIT=99,FILE='HERVH.TOP',STATUS='UNKNOWN')
...
c
      do j=1,2
      k=(j-1)*50
      call multitop(100+k+ 7,99,3,2,'H pt',' ','LOG')
      call multitop(100+k+ 8,99,3,2,'H pt',' ','LOG')
      call multitop(100+k+ 9,99,3,2,'H log[pt]',' ','LOG')
      call multitop(100+k+10,99,3,2,'H y',' ','LOG')
      call multitop(100+k+11,99,3,2,'H eta',' ','LOG')
      call multitop(100+k+12,99,3,2,'mH',' ','LOG')
      enddo
c
      do j=1,2
      k=(j-1)*50
      call multitop(100+k+13,99,3,2,'VH pt',' ','LOG')
      call multitop(100+k+14,99,3,2,'VH pt',' ','LOG')
      call multitop(100+k+15,99,3,2,'VH log[pt]',' ','LOG')
      call multitop(100+k+16,99,3,2,'VH y',' ','LOG')
      call multitop(100+k+17,99,3,2,'VH eta',' ','LOG')
      call multitop(100+k+18,99,3,2,'mVH',' ','LOG')
      call multitop(100+k+19,99,3,2,'VH azi',' ','LOG')
      call multitop(100+k+20,99,3,2,'VH azi',' ','LOG')
      call multitop(100+k+21,99,3,2,'VH deta',' ','LOG')
      enddo
c
      if(idec.eq.0)then
      do j=1,2
      k=(j-1)*50
      call multitop(100+k+25+ 1,99,3,2,'l pt',' ','LOG')
      call multitop(100+k+25+ 2,99,3,2,'l pt',' ','LOG')
      call multitop(100+k+25+ 3,99,3,2,'l log[pt]',' ','LOG')
      call multitop(100+k+25+ 4,99,3,2,'l eta',' ','LOG')
      call multitop(100+k+25+ 5,99,3,2,'l pt',' ','LOG')
      call multitop(100+k+25+ 6,99,3,2,'l pt',' ','LOG')
      call multitop(100+k+25+ 7,99,3,2,'l log[pt]',' ','LOG')
      call multitop(100+k+25+ 8,99,3,2,'l eta',' ','LOG')
c                          
      call multitop(100+k+25+ 9,99,3,2,'llb deta',' ','LOG')
      call multitop(100+k+25+10,99,3,2,'llb azi',' ','LOG')
      call multitop(100+k+25+11,99,3,2,'llb azi',' ','LOG')
      call multitop(100+k+25+12,99,3,2,'llb inv m',' ','LOG')
      call multitop(100+k+25+13,99,3,2,'llb pt',' ','LOG')
      call multitop(100+k+25+14,99,3,2,'llb pt',' ','LOG')
      enddo
      endif
c
      CLOSE(99)
...
c
      ...
      call mfill(kk+6,sngl(ppv(5)),sngl(WWW0))
c
      call mfill(kk+7,sngl(pth),sngl(WWW0))
      call mfill(kk+8,sngl(pth),sngl(WWW0))
      if(pth.gt.0.d0)call mfill(kk+9,sngl(log10(pth)),sngl(WWW0))
      call mfill(kk+10,sngl(yh),sngl(WWW0))
      call mfill(kk+11,sngl(etah),sngl(WWW0))
      call mfill(kk+12,sngl(pph(5)),sngl(WWW0))
c
      call mfill(kk+13,sngl(ptvh),sngl(WWW0))
      call mfill(kk+14,sngl(ptvh),sngl(WWW0))
      if(ptvh.gt.0.d0)call mfill(kk+15,sngl(log10(ptvh)),sngl(WWW0))
      call mfill(kk+16,sngl(yvh),sngl(WWW0))
      call mfill(kk+17,sngl(etavh),sngl(WWW0))
      call mfill(kk+18,sngl(xmvh),sngl(WWW0))
      call mfill(kk+19,sngl(azivh),sngl(WWW0))
      if(azivhnorm.gt.0.d0)
     #  call mfill(kk+20,sngl(log10(azivhnorm)),sngl(WWW0))
      call mfill(kk+21,sngl(detavh),sngl(WWW0))
c
      if(idec.eq.0)then
        kk=25
        call mfill(kk+1,sngl(ptl),sngl(WWW0))
        call mfill(kk+2,sngl(ptl),sngl(WWW0))
        if(ptl.gt.0.d0)call mfill(kk+3,sngl(log10(ptl)),sngl(WWW0))
        call mfill(kk+4,sngl(etal),sngl(WWW0))
        call mfill(kk+5,sngl(ptlb),sngl(WWW0))
        call mfill(kk+6,sngl(ptlb),sngl(WWW0))
        if(ptlb.gt.0.d0)call mfill(kk+7,sngl(log10(ptlb)),sngl(WWW0))
        call mfill(kk+8,sngl(etalb),sngl(WWW0))
c
        call mfill(kk+9,sngl(detallb),sngl(WWW0))
        call mfill(kk+10,sngl(azill),sngl(WWW0))
        if(azillnorm.gt.0.d0)
     #    call mfill(kk+11,sngl(log10(azillnorm)),sngl(WWW0))
        call mfill(kk+12,sngl(xmll),sngl(WWW0))
        call mfill(kk+13,sngl(ptll),sngl(WWW0))
        if(ptll.gt.0)call mfill(kk+14,sngl(log10(ptll)),sngl(WWW0))
      endif
c
      kk=50
...
c
      if(idec.eq.0)then
        ...
        if(ptl.gt.20.d0)call mfill(kk+4,sngl(etal),sngl(WWW0))
        if(abs(etalb).lt.ycut)then
          call mfill(kk+5,sngl(ptlb),sngl(WWW0))
          call mfill(kk+6,sngl(ptlb),sngl(WWW0))
          if(ptlb.gt.0.d0)call mfill(kk+7,sngl(log10(ptlb)),sngl(WWW0))
        endif
        if(ptlb.gt.20.d0)call mfill(kk+8,sngl(etalb),sngl(WWW0))
c
     ...
     #      call mfill(kk+11,sngl(log10(azillnorm)),sngl(WWW0))
          ...
          call mfill(kk+13,sngl(ptll),sngl(WWW0))
          if(ptll.gt.0) 
     #      call mfill(kk+14,sngl(log10(ptll)),sngl(WWW0))
        endif
      endif
c
 999  END
