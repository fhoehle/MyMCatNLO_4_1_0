...
c Hard-coded htoice for Ht mode; for s- and t-htannel see freal()
      parameter (jch=3)
c
...
c Soft-virtual function have been derived for idr=3. See finiteWt.m
        ...
        xfac2(1)=svfac2(s,u,t,xm12,xm22,xmuf2h1,5)
        xnonfac2(1)=svnonfac_stef(s,u,t,xm12,xm22,xmuf2h1,5)
        xfac2(3)=svfac2(s,t,u,xm12,xm22,xmuf2h1,5)
        xnonfac2(3)=svnonfac_stef(s,t,u,xm12,xm22,xmuf2h1,5)
...
c
c
      subroutine frealhtme(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,
     #                     jproc,xmatout)
c Returns the real matrix elements squared for W+t mode, times the flux 
c factor, times the FKS damping factor: 
c   xii**2*(1-yi**2)=4*tk*uk/s**2    
c Only initial-state kinematics is used here; final-state collinear
c singularities will not occur for W+t
c
c The matrix elements are passed through the following arrays
c xmat(out/in) (i):
c jproc=1 --> gg processes
...
c
      xii=1-x
      if(jproc.eq.1)then
        call frealhtme_gg(s,xii,yi,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jch)
          xmatout(i)=xmatin(i)
        enddo
      elseif(jproc.eq.2)then
        call frealhtme_qq(s,xii,yi,cth2,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jch)
          xmatout(i)=xmatin(i)
        enddo
      elseif(jproc.eq.3)then
        call frealhtme_qg(s,xii,yi,cth2,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jch)
          xmatout(i)=xmatin(i)
        enddo
      else
        write(*,*)'Unknown process in frealhtme',jproc
        stop
      endif
c
      return
      ...
      subroutine frealhtme_gg(xs,xxii,xyi,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for gg --> tWbbar. See frealhtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xtk,xuk,xq1q,xq2q,xxinv(5),
     #  xmatout(7)
      real * 8 s,xii,yi,tk,uk,q1q,q2q,q1c,q2c,tiny,pi,vcf,
     #  s_red,t_red,x_ap,ap_kern,xmadevht,xinv(5),xmatin(7)
      real*8 xmom_cm(10,4),ymom(10,4)
...
c Collinear + limit
        ...
        s_red=s*x_ap
        t_red=q2q
        call getredmom(1,xmom_cm,ymom,s_red)
        call fbornhtme(s_red,ymom,ithree,xmatin) 
        do i=1,idrmax(ione,jch)
          if(idrlimcp(jch,ione,i).ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcp(jch,ione,i))
...
c use the full expression for the matrix element.
c Factor multiplying the matrix element, eq.(4.37)
        xmatin(1)=xmadevht(ione,ione,ione,xm12,xm22,s,tk,uk,
     #                     q1q,q2q,xmom_cm)
        do i=1,idrmax(ione,jch)
          xmatout(i)=xmatin(i)
        enddo
...
c
      ...
      subroutine frealhtme_qq(xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for qq --> tWbbar. See frealhtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,xxinv(5),xmatout(7)
      real * 8 s,xii,yi,cth2,tk,uk,q1q,q2q,q1c,q2c,tiny,pi,vcf,
     #  s_red,t_red,x_ap,ap_kern,qin_kern,xmadevht,xinv(5),xmatin(7),
     # xaziin(7)
      real*8 xmom_cm(10,4),ymom(10,4)
      common/cxmomcm/xmom_cm
      integer i,jch,ione,itwo,ithree,ipone,imone,icode,idrmax(1:3,3)
      common/cidrmax/idrmax
      integer idrlimcp(3,1:3,8),idrlimcm(3,1:3,8)
      common/cidrlims/idrlimcp,idrlimcm
      parameter (tiny=1.d-4)
...
c Collinear + limit
        ...
        call getredmom(1,xmom_cm,ymom,s_red)
        call fbornhtme(s_red,ymom,ithree,xmatin) 
        call fazihtme(s_red,t_red,cth2,itwo,ipone,xaziin)
        do i=1,idrmax(itwo,jch)
          if(idrlimcp(jch,itwo,i).ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcp(jch,itwo,i)) +
...
c Collinear - limit
        ...
        call getredmom(-1,xmom_cm,ymom,s_red)
        call fbornhtme(s_red,ymom,ithree,xmatin) 
        call fazihtme(s_red,t_red,cth2,itwo,imone,xaziin)
        do i=1,idrmax(itwo,jch)
          if(idrlimcm(jch,itwo,i).ne.0)
     #      xmatout(i)=4*(1-yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcm(jch,itwo,i)) +
...
c Factor multiplying the matrix element, eq.(4.37)
        do i=1,idrmax(itwo,jch)
          xmatout(i)=xmadevht(ione,itwo,i,xm12,xm22,s,tk,uk,
     #                        q1q,q2q,xmom_cm)
        enddo
      endif
c
      ...
      subroutine frealhtme_qg(xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for qg --> tWbbar. See frealhtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,xxinv(5),
     # xmatout(7)
      real * 8 s,xii,yi,cth2,tk,uk,q1q,q2q,q1c,q2c,tiny,pi,vcf,vca,
     # s_red,t_red,x_ap,ap_kern,qin_kern,xmadevht,xinv(5),xmatin(7),
     # softfc(3),xaziin(7)
      real*8 xmom_cm(10,4),ymom(10,4)
      common/cxmomcm/xmom_cm
      integer i,jch,ione,ithree,ipone,imone,icode,idrmax(1:3,3)
      common/cidrmax/idrmax
      integer iapcp(3,7),iapcm(3,7)
      common/ciap/iapcp,iapcm
      integer idrlimcp(3,1:3,8),idrlimcm(3,1:3,8)
...
c Collinear + limit
        ...
        call getredmom(1,xmom_cm,ymom,s_red)
        call fbornhtme(s_red,ymom,ithree,xmatin) 
        call fazihtme(s_red,t_red,cth2,ithree,ipone,xaziin)
        do i=1,idrmax(ithree,jch)
          icode=iapcp(jch,i)
          if(idrlimcp(jch,ithree,i).ne.0.and.icode.ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
...
c Collinear - limit
        ...
        call getredmom(-1,xmom_cm,ymom,s_red)
        call fbornhtme(s_red,ymom,ithree,xmatin) 
        call fazihtme(s_red,t_red,cth2,ithree,imone,xaziin)
        do i=1,idrmax(ithree,jch)
          icode=iapcm(jch,i)
          if(idrlimcm(jch,ithree,i).ne.0.and.icode.ne.0)
     #      xmatout(i)=4*(1-yi)/s*ap_kern(x_ap,abs(icode))*
...
c Factor multiplying the matrix element, eq.(4.37)
        do i=1,idrmax(ithree,jch)
          if(i.ne.2)xmatout(i)=xmadevht(ione,ithree,i,xm12,xm22,
     #                                  s,tk,uk,q1q,q2q,xmom_cm)
        enddo
      endif
      return
      end
      ...
      subroutine fbornhtme(s,xmom,jproc,xmatout)
c Returns the partonic Born contribution, with the following normalization
c
...
c   u=(p1-k2)^2=(p2-k1)^2
      implicit none
      include 'stpcblks.h'
      real*8 xmom(10,4),s,zero,xmatout(7)
      parameter (zero=0.d0)
      integer jproc,i,izero,ione,ithree,jch,idrmax(1:3,3)
      common/cidrmax/idrmax
      parameter (izero=0)
      parameter (ione=1)
      parameter (ithree=3)
...
c
      if(jproc.lt.1.or.jproc.gt.3)then
        write(*,*)'Error in fbornhtme: jproc=',jproc
        stop
      endif
c
      do i=1,idrmax(jproc,jch)
...
c
      if(jproc.eq.3)then
        xmatout(1)=xmadevht(izero,ithree,ione,xm12,xm22,
     #                      s,zero,zero,zero,zero,xmom)
        xmatout(3)=xmadevht(izero,ithree,ithree,xm12,xm22,
     #                      s,zero,zero,zero,zero,xmom)
      endif
c
      return
      ...
      function azidepme(s,t,u,m2,q2,cth2)
c Returns the azimuthal-dependent part for the y=-1 collinear singularity
c of any real process that factorizes
c  b(p1)+g(p2) --> t(k1)+W(k2) 
c The definitions of the invariants and the normalizations are as in bgborn()
c Identical to azidep(); the string me is appended to avoid clashing names
      implicit none
      real * 8 azidepme,s,t,u,m2,q2,cth2
c
     ...
     & u - m2)/(u - m2)*m2**2*q2*s**(-1)*cth2**2
      ...
      subroutine fazihtme(s,t,cth2,jproc,icoll,xmatout)
c Returns the azimuthal-dependent part of collinear limits; the definitions
c are as in fbornht.
c Identical to faziht(); the string me is appended to avoid clashing names
      implicit none
      include 'stpcblks.h'
      real*8 s,t,cth2,xmatout(7)
      real*8 vtr,xnc,xnorm,u,azidepme,azi1,azi3
      parameter (vtr=0.5d0)
      parameter (xnc=3.d0)
      integer jproc,icoll,i,jch,idrmax(1:3,3)
      common/cidrmax/idrmax
...
c
      if(jproc.lt.2.or.jproc.gt.3)then
        write(*,*)'Error in fazihtme: jproc=',jproc
        stop
      endif
c
      do i=1,idrmax(jproc,jch)
...
c
      xnorm=vtr/(4*xnc)
      u=xm12+xm22-s-t
      azi1=xnorm*azidepme(s,t,u,xm12,xm22,cth2)/(2*s)
      azi3=xnorm*azidepme(s,u,t,xm12,xm22,cth2)/(2*s)
      if(icoll.eq.1)then
        if(jproc.eq.2)then
          xmatout(4)=azi3
          xmatout(6)=azi3
      ...
      elseif(icoll.eq.-1)then
        ...
        write(*,*)'Error in fazihtme: icoll=',icoll
        stop
      endif
c
      return
      ...
      function xmadevht(iborn,jproc,idr,xm12,xm22,s,tk,uk,q1q,q2q,xmom)
...
c MadEvent routines use the SM parameters as defined in setmeFRpar();
...
c to g^2 (A^2+B^2) [g^4 (A^2+B^2)] (neglecting a CKM factor, which
c is trivial for unweighting purposes), and this call can be placed somewhere
c else (and done only once). The main code assumes that g^n (A^2+B^2) 
c be stripped off matrix element routines, which is equivalent to setting
c g, and (A^2+B^2) equal to one. However, the call to setmeFRpar() does not 
c set (A^2+B^2)=1, and thus we divide here by (A^2+B^2).
...
c in the tHD vertex
      ...
      real*8 xmadevht,xm12,xm22,s,tk,uk,q1q,q2q,xmom(10,4),xfact,tmp(1)
      real*8 pme0(0:3,4),pme1(0:3,5)
      real*8 tiny,w2,oosqbw
      parameter (tiny=1.d-4)
      real*8 wt0,xbwfact
      common/cwt0BW/wt0,xbwfact
      real*8 xmt,twidth
...
c Subprocess: MC@NLO   -> x(1)y(2)  ->  z(3)t(4)h-(5)
c             MadEvent -> b(1)g(2)  ->  t(3)h-(4)
c             MadEvent -> g(1)g(2)  ->  t(3)h-(4)b(5)
c             MadEvent -> b(1)bx(2) ->  t(3)h-(4)bx(5)
c             MadEvent -> b(1)g(2)  ->  t(3)h-(4)g(5)
      integer mapdir(5),mapref(5)
c mapping for directed events
      data mapdir/1,2,4,5,3/
c mapping for reflected events
      data mapref/2,1,4,5,3/
c
      if(iborn.eq.0)then
        if(jproc.ne.3)then
     ...
     #      (jproc.eq.3.and.(idr.ne.1.and.idr.ne.3)) )then
        ...
        w2=xm22-q2q+q1q-uk
        wt0=tiny
        xbwfact=oosqbw(w2,xm12,wt0)/oosqbw(w2,xm12,twidth)
        do ipart=1,5
          do icomp=0,3
            if( jproc.eq.1 .or.
     ...
     #       (idr.eq.5.or.idr.eq.6))then
      ...
      xmadevht=1/apbsq*xfact*tmp(1)/(2*s)
      return
      end


      function oosqbw(q2,xm2,gamma)
      implicit none
      real*8 oosqbw,q2,xm2,gamma
c
      oosqbw=sqrt( (q2-xm2)**2+xm2*gamma**2 )
      return
      end
...
C  
C FOR PROCESS : b g -> t h-  
C  
C Crossing   1 is b g -> t h-  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      REAL*8 FUNCTION MATBGTH(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c      WT=0d0
      ...
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,MT      ,ZERO      ,W(1,5   ))    
      CALL IOSXXX(W(1,1   ),W(1,5   ),W(1,4   ),HMCOUP ,AMP(1   ))         
      CALL FVIXXX(W(1,1   ),W(1,2   ),GG ,MB      ,ZERO    ,W(1,6   ))     
      CALL IOSXXX(W(1,6   ),W(1,3   ),W(1,4   ),HMCOUP ,AMP(2   ))         
      JAMP(   1) = +AMP(   1)+AMP(   2)
      MATBGTH = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          MATBGTH =MATBGTH+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : g g -> t h- b~  
C  
C Crossing   1 is g g -> t h- b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      REAL*8 FUNCTION MATGGTHB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL OXXXXX(P(0,3   ),MT ,NHEL(3   ),+1*IC(3   ),W(1,3   ))
      CALL SXXXXX(P(0,4   ),+1*IC(4   ),W(1,4   ))                    
      CALL IXXXXX(P(0,5   ),MB ,NHEL(5   ),-1*IC(5   ),W(1,5   ))     
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,MT      ,WT0     ,W(1,6   ))
      CALL FSOXXX(W(1,6   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,  
     &     7   ))
      CALL IOVXXX(W(1,5   ),W(1,7   ),W(1,1   ),GG ,AMP(1   ))           
      CALL FVOXXX(W(1,6   ),W(1,1   ),GG ,MT      ,WT0     ,W(1,8   ))
      CALL IOSXXX(W(1,5   ),W(1,8   ),W(1,4   ),HMCOUP ,AMP(2   ))    
      CALL FVOXXX(W(1,3   ),W(1,1   ),GG ,MT      ,WT0     ,W(1,9   ))
      CALL FSOXXX(W(1,9   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,  
     &     10  ))  
      CALL IOVXXX(W(1,5   ),W(1,10  ),W(1,2   ),GG ,AMP(3   ))         
      CALL FVOXXX(W(1,9   ),W(1,2   ),GG ,MT      ,WT0     ,W(1,11  ))
      CALL IOSXXX(W(1,5   ),W(1,11  ),W(1,4   ),HMCOUP ,AMP(4   ))    
      CALL FSOXXX(W(1,3   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,  
     &     12  ))     
      CALL FVIXXX(W(1,5   ),W(1,1   ),GG ,MB      ,ZERO    ,W(1,13  ))
      CALL IOVXXX(W(1,13  ),W(1,12  ),W(1,2   ),GG ,AMP(5   ))        
      CALL FVIXXX(W(1,5   ),W(1,2   ),GG ,MB      ,ZERO    ,W(1,14  ))
      CALL IOVXXX(W(1,14  ),W(1,12  ),W(1,1   ),GG ,AMP(6   ))           
      CALL JVVXXX(W(1,1   ),W(1,2   ),MGVX3 ,ZERO    ,ZERO    ,W(1,   
     &     15  ))                                                   
      CALL FVOXXX(W(1,3   ),W(1,15  ),GG ,MT      ,WT0     ,W(1,16  ))
      CALL IOSXXX(W(1,5   ),W(1,16  ),W(1,4   ),HMCOUP ,AMP(7   ))    
      CALL IOVXXX(W(1,5   ),W(1,12  ),W(1,15  ),GG ,AMP(8   ))        
      JAMP(   1) = -AMP(   1)-AMP(   5)+AMP(   8) +
     #             xbwfact*( -AMP(   2)+AMP(   7) )
      JAMP(   2) = -AMP(   3)-AMP(   6)-AMP(   8) +
     #             xbwfact*( -AMP(   4)-AMP(   7) )
      MATGGTHB = 0.D0
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTHB =MATGGTHB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN)
      END
     

      SUBROUTINE BGTHG(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : b g -> t h- g  
C  
C Crossing   1 is b g -> t h- g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      REAL*8 FUNCTION MATBGTHG(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,MT      ,WT      ,W(1,6   ))     
      CALL FSOXXX(W(1,6   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,       
     &     7   ))                                                          
      CALL IOVXXX(W(1,1   ),W(1,7   ),W(1,5   ),GG ,AMP(1   ))             
      CALL FSIXXX(W(1,1   ),W(1,4   ),HMCOUP ,MT      ,WT      ,W(1,       
     &     8   ))                                                          
      CALL JVVXXX(W(1,5   ),W(1,2   ),MGVX3 ,ZERO    ,ZERO    ,W(1,        
     &     9   ))                                                          
      CALL IOVXXX(W(1,8   ),W(1,3   ),W(1,9   ),GG ,AMP(2   ))             
      CALL FVOXXX(W(1,3   ),W(1,5   ),GG ,MT      ,WT      ,W(1,10  ))     
      CALL IOVXXX(W(1,8   ),W(1,10  ),W(1,2   ),GG ,AMP(3   ))             
      CALL IOVXXX(W(1,8   ),W(1,6   ),W(1,5   ),GG ,AMP(4   ))             
      CALL FSOXXX(W(1,3   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,       
     &     11  ))                                                          
      CALL FVIXXX(W(1,1   ),W(1,5   ),GG ,MB      ,ZERO    ,W(1,12  ))     
      CALL IOVXXX(W(1,12  ),W(1,11  ),W(1,2   ),GG ,AMP(5   ))             
      CALL IOVXXX(W(1,1   ),W(1,11  ),W(1,9   ),GG ,AMP(6   ))             
      CALL FVIXXX(W(1,1   ),W(1,2   ),GG ,MB      ,ZERO    ,W(1,13  ))     
      CALL FSIXXX(W(1,13  ),W(1,4   ),HMCOUP ,MT      ,WT      ,W(1,       
     &     14  ))                                                          
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBGTHG =MATBGTHG+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : b b~ -> t h- b~  
C  
C Crossing   1 is b b~ -> t h- b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTHBX(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION MATBBXTHBX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL SXXXXX(P(0,4   ),+1*IC(4   ),W(1,4   ))                         
      CALL IXXXXX(P(0,5   ),MB ,NHEL(5   ),-1*IC(5   ),W(1,5   ))          
      CALL FSIXXX(W(1,1   ),W(1,4   ),HMCOUP ,MT      ,WT0       ,W(1,       
     &     6   ))                                                          
     ...
     &     8   ))                                                          
      ...
      CALL FVOXXX(W(1,3   ),W(1,9   ),GG ,MT      ,WT0     ,W(1,10  ))     
      CALL IOSXXX(W(1,5   ),W(1,10  ),W(1,4   ),HMCOUP ,AMP(3   ))         
      CALL IOVXXX(W(1,5   ),W(1,8   ),W(1,9   ),GG ,AMP(4   ))             
      JAMP(   1) = +AMP(   1)+AMP(   2)
      JAMP(   2) = -xbwfact*AMP(   3)-AMP(   4)
      MATBBXTHBX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTHBX =MATBBXTHBX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
       

      SUBROUTINE BUTHU(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : b u -> t h- u  
C  
C Crossing   1 is b u -> t h- u  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      REAL*8 FUNCTION MATBUTHU(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FSIXXX(W(1,1   ),W(1,4   ),HMCOUP ,MT      ,ZERO      ,W(1,         
     &     6   ))                                                          
     ...
     &     8   ))                                                          
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBUTHU =MATBUTHU+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
...
C ----------
      ...
      REAL*8 FUNCTION MATBBTHB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
     ...
     &     8   ))                                                          
      CALL IOVXXX(W(1,1   ),W(1,8   ),W(1,7   ),GG ,AMP(2   ))             
      CALL FSIXXX(W(1,2   ),W(1,4   ),HMCOUP ,MT      ,ZERO      ,W(1,         
     &     9   ))                                                          
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBTHB =MATBBTHB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : u u~ -> t h- b~  
C  
C Crossing   1 is u u~ -> t h- b~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTHBX(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION MATUUXTHBX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FVOXXX(W(1,3   ),W(1,6   ),GG ,MT      ,WT0     ,W(1,7   ))     
      CALL IOSXXX(W(1,5   ),W(1,7   ),W(1,4   ),HMCOUP ,AMP(1   ))           
      CALL FSOXXX(W(1,3   ),W(1,4   ),HMCOUP ,MB      ,ZERO    ,W(1,         
     &     8   ))                                                          
      CALL IOVXXX(W(1,5   ),W(1,8   ),W(1,6   ),GG ,AMP(2   ))             
      JAMP(   1) = -xbwfact*AMP(   1)-AMP(   2)
      MATUUXTHBX = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTHBX =MATUUXTHBX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END


      subroutine getredmom(itype,xmom,ymom,s)
c itype=0,1,-1 ==> soft,collinear+,collinear-
c Ignore momenta xmom(i,*) with i>5, which are unused in the computation
c of the matrix elements
      implicit none
      real*8 xmom(10,4),ymom(10,4),s
      integer itype,i
c
      if(itype.eq.1)then
        do i=1,4
          ymom(1,i)=xmom(1,i)-xmom(3,i)
          ymom(2,i)=xmom(2,i)
          ymom(3,i)=0.d0
          ymom(4,i)=xmom(4,i)
          ymom(5,i)=xmom(5,i)
      ...
      elseif(itype.eq.-1)then
        do i=1,4
          ymom(1,i)=xmom(1,i)
          ymom(2,i)=xmom(2,i)-xmom(3,i)
          ymom(3,i)=0.d0
          ymom(4,i)=xmom(4,i)
          ymom(5,i)=xmom(5,i)
        enddo
      else
        write(*,*)'Unknown option in getredmom',itype
        stop
      endif
      s=2*( ymom(1,4)*ymom(2,4)-ymom(1,1)*ymom(2,1)-
     #      ymom(1,2)*ymom(2,2)-ymom(1,3)*ymom(2,3) )
      return
      end


      subroutine frealhtsub(jproc,xm12,xm22,s,x,y,cth1,cth2,
     #                      tk,uk,q1q,q2q,x1,x2,x1p,x2p,xmatout)
c Wrapper for xmadsubht. See comments there
      implicit none
      real*8 xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,x1,x2,
     # x1p,x2p,xmadsubht,xmatout(7)
      integer jproc,i,iborn,ione,itwo,ifive,isix
      parameter (iborn=1)
      parameter (ione=1)
      parameter (itwo=2)
      parameter (ifive=5)
...
c
      ...
      enddo
      if(jproc.eq.1)then
        xmatout(1)=xmadsubht(iborn,jproc,ione,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
      elseif(jproc.eq.2)then
        xmatout(1)=xmadsubht(iborn,jproc,ione,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(2)=xmadsubht(iborn,jproc,itwo,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(5)=xmadsubht(iborn,jproc,ifive,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(6)=xmadsubht(iborn,jproc,isix,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
      else
        x1p=1.d0
        x2p=1.d0
      ...
      function xmadsubht(iborn,jproc,idr,xm12,xm22,s,x,y,cth1,cth2,
     #                   tk,uk,q1q,q2q,x1,x2,x1p,x2p)
c Computes the gg->tHbbar and qqbar/bbbar->tHbbar contributions, including
c only the doubly-resonant diagrams. This is done by assuming the 
c processes above are actually gg->t(tbar->Hbbar). The original kinematics
c is reshuffled in such a way that t and tbar are both on shell. The
c decay tbar->Hbbar is performed in the same manner as done in the
c ttbar package. The call to Madevent routines ensures that spin correlations
c are correctly taken into account.
c The same routine can be used for the charge-conjugated processes 
c gg->tbarHb and qqbar/bbbar->tbarHb. In such a case the decay is t->Hb.
c Therefore, it would be more appropriate to denote the original final-state
c on-shell t or tbar are tagged-t, and the virtual tbar or t producing the
c H-b pair as off-shell t. In the comments in the following routines,
c we always called the tagged-t as t, and the off-shell t as tbar.
c This routine is derived from xmadevht
      implicit none
      integer iborn,jproc,idr
      real*8 xmadsubht,xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,
     # x1,x2,x1p,x2p,xfact,w2,bwsupp,bwfunc,sp,staup,xlstaup,ycmp,
     # tk0,uk0,q1q0,q2q0,one,cth1p,cth2p,cthbw,phibw,xinv0(5),
     # pme1(0:3,5),tmp(1)
      parameter (one=1.d0)
      character*2 str
      parameter (str='p1')
...
c             MadEvent -> b(1)g(2)  ->  t(3)H-(4)
c             MadEvent -> g(1)g(2)  ->  t(3)H-(4)b(5)
c             MadEvent -> b(1)u(2)  ->  t(3)H-(4)u(5)
c             MadEvent -> b(1)bx(2) ->  t(3)H-(4)bx(5)
c             MadEvent -> b(1)g(2)  ->  t(3)H-(4)g(5)
      integer mapdir(5),mapref(5)
c mapping for directed events
...
c
      if(iborn.eq.0)then
        write(*,*)'Error #1 in xmadsubht: iborn, jproc=',iborn,jproc
        stop
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.2.and.
     ...
     #      jproc.eq.3 )then
          write(*,*)'Error #2 in xmadsubht: jproc,idr=',jproc,idr
          stop
        endif
c FKS damping factor
        xfact=4*tk*uk/s**2
c bH invariant mass -- ie off-shell top virtuality
        w2=xm22-q2q+q1q-uk
        wt0=tiny
        xbwfact=oosqbw(xm12,xm12,wt0)/oosqbw(xm12,xm12,twidth)
c Suppression factor for off-shell contributions modelled by 
c Breit-Wigner factors
        bwsupp=bwfunc(w2,xm12,twidth)/bwfunc(xm12,xm12,twidth)
        sp=min( ((2*sqrt(xm12))/(sqrt(xm12)+sqrt(w2)))**2*s , sh )
        staup=sqrt(sp/sh)
        xlstaup=log(staup)
        ycmp=0.5d0*log(x1/x2)
        if(ycmp.le.xlstaup)ycmp=0.999*xlstaup
        if(ycmp.ge.-xlstaup)ycmp=-0.999*xlstaup
        x1p=staup*exp(ycmp)
        x2p=staup/exp(ycmp)
c Generate kinematics with both tops on-shell 
        call gettopdir(cth1p,cth2p)
        call getbwang(cthbw,phibw)
        call invar_in(xm12,xm12,sp,one,y,cth1p,cth2p,str,
     #                tk0,uk0,q1q0,q2q0,xinv0)
        call genbwmom(xm12,xm22,cthbw,phibw)
        if(ichkmom.eq.0)call checkmom(xmom_cm,sp,ycmp,4,2)
        do ipart=1,5
          do icomp=0,3
            if( jproc.eq.1 .or.
     #          (jproc.eq.2.and.(idr.eq.1.or.idr.eq.3.or.
     #                          idr.eq.5.or.idr.eq.7)) .or.
     #          (jproc.eq.3.and.idr.eq.1) )then
              pme1(icomp,ipart)=xmom_cm(mapdir(ipart),mapcomp(icomp))
            else
              pme1(icomp,ipart)=xmom_cm(mapref(ipart),mapcomp(icomp))
            endif
          enddo
        enddo
c Restore original kinematics (because of common blocks) 
        call invar_in(xm12,xm22,s,x,y,cth1,cth2,str,
     #                tk0,uk0,q1q0,q2q0,xinv0)
      else
        write(*,*)'xmadevht: unknown iborn value',iborn
        stop
      endif
      if(iborn.eq.1.and.jproc.eq.1)then
        call ggthb_s(pme1,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.1.or.idr.eq.2))then
        call uuxthbx_s(pme1,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.5.or.idr.eq.6))then
        call bbxthbx_s(pme1,tmp)
      else
        write(*,*)'xmadsubht: Error #3'
        stop
      endif
      xmadsubht=1/apbsq*bwsupp*xfact*tmp(1)/(2*s)
      return
      end


      subroutine gettopdir(cth1p,cth2p)
c Computes polar (whose cosine is cth1p) and azimuthal angles (whose 
c cosine is cth2p) of the top in the parton CM frame. Consistenly with
c FNR, the top momentum has direction 
c   ( sin(th2)*sin(th1) , cos(th2)*sin(th1) , cos(th1) )
c The top momentum is xmom_cm(4,*)
      implicit none
      real*8 cth1p,cth2p,xnorm,th2p
      real*8 xmom_cm(10,4)
      common/cxmomcm/xmom_cm
c
      xnorm=xmom_cm(4,1)**2+xmom_cm(4,2)**2+xmom_cm(4,3)**2
      if(xnorm.gt.1.d-8)then
        xnorm=sqrt(xnorm)
        cth1p=xmom_cm(4,3)/xnorm
        th2p=atan2(xmom_cm(4,1),xmom_cm(4,2))
        cth2p=cos(th2p)
      else
        cth1p=1.d0
        cth2p=1.d0
      ...
      subroutine getbwang(cthbw,phibw)
c Computes the polar and azimuthal angles of the W and b in the tbar
c rest frame. The W momentum is xmom_cm(5,*),the b momentum is xmom_cm(3,*), 
c and thus the tbar momentum is xmom_cm(3,*)+xmom_cm(5,*). In the tbar rest 
c frame, the W momentum has direction
c   ( cos(phi)*sin(th) , sin(phi)*sin(th) , cos(th) )
c which defines the angles to be computed. The axis system is that boosted
c from the parton CM frame
      implicit none
      real*8 cthbw,phibw,xnorm,tbar(5),xw(4),xwb(4)
      integer i
      real*8 xmom_cm(10,4)
      common/cxmomcm/xmom_cm
c
      do i=1,4
        xw(i)=xmom_cm(5,i)
        tbar(i)=xmom_cm(3,i)+xmom_cm(5,i)
      enddo
      tbar(5)=sqrt(tbar(4)**2-tbar(1)**2-tbar(2)**2-tbar(3)**2)
c Boost W momentum to tbar rest frame
      call hwulf4(tbar,xw,xwb)
      xnorm=xwb(1)**2+xwb(2)**2+xwb(3)**2
      if(xnorm.gt.1.d-8)then
        xnorm=sqrt(xnorm)
        cthbw=xwb(3)/xnorm
        phibw=atan2(xwb(2),xwb(1))
      else
        cthbw=1.d0
        phibw=0.d0
      endif
      ...
      subroutine genbwmom(xmt2,xmw2,cthbw,phibw)
c Generates the four-momenta of the W and b after the tbar has been put
c on shell. These four momenta are stored in xmom_cm(5,*) and xmom_cm(3,*);
c previous entries in this array are overwritten, and no warning is given.
c When this function is called, xmom_cm(5,*) must be the tbar four-momentum,
c and xmom_cm(3,*) must be void. This routine has been derived from
c gentopdmom of the ttbar package
      implicit none
      real*8 xmt2,xmw2,cthbw,phibw,xm22,xmt,sthbw,ew,eb,pwx,pwy,pwz,
     # pbx,pby,pbz,tbar(5),xw(4),xbq(4)
      real*8 xmom_cm(10,4)
      common/cxmomcm/xmom_cm
      integer ib,iw
      parameter (ib=3)
      parameter (iw=5)
c
      xm22=xmom_cm(5,4)**2-xmom_cm(5,1)**2-
     #     xmom_cm(5,2)**2-xmom_cm(5,3)**2
      if(abs(xm22-xmt2).gt.1.d-6)then
        write(*,*)'Recoiling top not on shell'
        stop
      endif
      xmt=sqrt(xmt2)
      sthbw=sqrt(1-cthbw**2)
c W and b momenta, top rest frame
      ew=(xmt2+xmw2)/(2*xmt)
      eb=(xmt2-xmw2)/(2*xmt)
      pwx=eb*sthbw*cos(phibw)
      pwy=eb*sthbw*sin(phibw)
      pwz=eb*cthbw
      pbx=-pwx
      pby=-pwy
      pbz=-pwz
      ...
      xbq(3)=pbz
      xbq(4)=eb
c Boost all momenta to parton CM frame
      tbar(1)=xmom_cm(5,1)
      tbar(2)=xmom_cm(5,2)
      tbar(3)=xmom_cm(5,3)
      tbar(4)=xmom_cm(5,4)
      tbar(5)=xmt
c
      call filltopdec(tbar,xw,iw)
      call filltopdec(tbar,xbq,ib)
c
      return
      end


      subroutine fluxrwgt(x1p,x2p,sf,rsf)
      implicit none
      real*8 x1p,x2p,sf(7,3,24,2),rsf(7,3,24,2)
      real*8 sfp(7,3,24,2)
      integer jch,jproc,j,itype,itt
      parameter (jch=3)
      integer ifluxrwgt
...
c
      if(ifluxrwgt.eq.0.and.x1p.lt.1.d0.and.x2p.lt.1.d0)
     #  call strfunht(x1p,x2p,sfp)
      do jproc=loproc,maproc
        do j=1,idrmax(jproc,jch)
          do itype=1,jtypemax(jproc,j)
            do itt=ittmin,ittmax
              if(ifluxrwgt.eq.0)then
                if( sf(j,jproc,itype,itt).gt.1.d-8.and.
     #              x1p.lt.1.d0.and.x2p.lt.1.d0 )then
                  rsf(j,jproc,itype,itt)=sfp(j,jproc,itype,itt)/
     #                                   sf(j,jproc,itype,itt)
                else
                  rsf(j,jproc,itype,itt)=0.d0
                endif
              else
                rsf(j,jproc,itype,itt)=1.d0
              endif
            enddo
          enddo
        enddo
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATGGTHB_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION MATGGTHB_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL IXXXXX(P(0,5   ),MB ,NHEL(5   ),-1*IC(5   ),W(1,5   ))          
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,MT      ,WT0     ,W(1,6   ))     
      CALL FVOXXX(W(1,6   ),W(1,1   ),GG ,MT      ,WT0     ,W(1,7   ))     
      CALL IOSXXX(W(1,5   ),W(1,7   ),W(1,4   ),HMCOUP ,AMP(1   ))           
      CALL FVOXXX(W(1,3   ),W(1,1   ),GG ,MT      ,WT0     ,W(1,8   ))     
      CALL FVOXXX(W(1,8   ),W(1,2   ),GG ,MT      ,WT0     ,W(1,9   ))     
      CALL IOSXXX(W(1,5   ),W(1,9   ),W(1,4   ),HMCOUP ,AMP(2   ))           
      CALL JVVXXX(W(1,1   ),W(1,2   ),MGVX3 ,ZERO    ,ZERO    ,W(1,        
     &     10  ))                                                          
      CALL FVOXXX(W(1,3   ),W(1,10  ),GG ,MT      ,WT0     ,W(1,11  ))     
      CALL IOSXXX(W(1,5   ),W(1,11  ),W(1,4   ),HMCOUP ,AMP(3   ))           
      JAMP(   1) = xbwfact*( -AMP(   1)+AMP(   3) )
      JAMP(   2) = xbwfact*( -AMP(   2)-AMP(   3) )
      MATGGTHB_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTHB_S =MATGGTHB_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
       

      SUBROUTINE BBXTHBX_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTHBX_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION MATBBXTHBX_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTHBX_S =MATBBXTHBX_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END


      SUBROUTINE UUXTHBX_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTHBX_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
      ...
      REAL*8 FUNCTION MATUUXTHBX_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTHBX_S =MATUUXTHBX_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
       

      function xmadevht_dc(iborn,jproc,idr,s,tk,uk,xmom)
c Wrapper for MadEvent functions. Inputs are 
c   iborn = 0(born), 1(real)
c   jproc = 1(gg), 2(qq), 3(qg)
c   idr   = 1..7 (depends on jproc)
...
c in the tHD vertex
      implicit none
      integer iborn,jproc,idr
      real*8 xmadevht_dc,s,tk,uk,xmom(10,4),xfact,tmp(1)
...
c
      if(iborn.eq.0)then
        if(jproc.ne.3)then
          write(*,*)'Error #1 in xmadevht_dc: iborn, jproc=',
     #              iborn,jproc
          stop
        endif
        xfact=1.d0
        do ipart=1,6
          do icomp=0,3
            ...
            elseif(idr.eq.3)then
              ...
              write(*,*)'Error #2 in xmadevht_dc: idr=',idr
              stop
            endif
          enddo
        enddo
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.3.and.(idr.ne.1.and.idr.ne.3)) )then
          write(*,*)'Error #3 in xmadevht_dc: jproc,idr=',jproc,idr
          stop
        endif
        xfact=4*tk*uk/s**2
        do ipart=1,7
     ...
     #          (jproc.eq.3.and.idr.eq.1) )then
        ...
        write(*,*)'xmadevht_dc: unknown iborn value',iborn
        stop
      endif
      if(iborn.eq.0)then
...
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      ...
      function xmadsubht_dc(iborn,jproc,idr,xm12,xm22,s,x,y,cth1,cth2,
     #                      tk,uk,q1q,q2q,x1,x2,x1p,x2p,
     #                      q1,cthtq1,phitq1,cthtq2,phitq2)
c Computes the gg->tHbbar and qqbar/bbbar->tHbbar contributions, including
c only the doubly-resonant diagrams. This is done by assuming the 
c processes above are actually gg->t(tbar->Hbbar). The original kinematics
c is reshuffled in such a way that t and tbar are both on shell. The
...
c Therefore, it would be more appropriate to denote the original final-state
c on-shell t or tbar are tagged-t, and the virtual tbar or t producing the
c W-b pair as off-shell t. In the comments in the following routines,
c we always called the tagged-t as t, and the off-shell t as tbar.
c This routine is derived from xmadevht
      implicit none
      integer iborn,jproc,idr
      real*8 xmadsubht_dc,xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,
     # x1,x2,x1p,x2p,xfact,w2,bwsupp,bwfunc,sp,staup,xlstaup,ycmp,
     # tk0,uk0,q1q0,q2q0,one,cth1p,cth2p,cthbw,phibw,xinv0(5),
     # pme1dc(0:3,8),tmp(1)
      real*8 q1,cthtq1,phitq1,cthtq2,phitq2
      real*8 xtq(4),xbq(4),xtl(4),xtn(4)
      parameter (one=1.d0)
      character*2 str
      parameter (str='p1')
      real*8 xmom_cm(10,4)
...
c
      if(iborn.eq.0)then
        write(*,*)'Error #1 in xmadsubht_dc: iborn, jproc=',iborn,jproc
        stop
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.2.and.
     #       (idr.ne.1.and.idr.ne.2.and.idr.ne.5.and.idr.ne.6)) .or.
     #      jproc.eq.3 )then
          write(*,*)'Error #2 in xmadsubht_dc: jproc,idr=',jproc,idr
          stop
        endif
c FKS damping factor
        xfact=4*tk*uk/s**2
...
c Generate kinematics with both tops on-shell 
     ...
     #                tk0,uk0,q1q0,q2q0,xinv0)
        call genbwmom(xm12,xm22,cthbw,phibw)
        if(ichkmom.eq.0)call checkmom(xmom_cm,sp,ycmp,4,2)
c Generate momenta of top decay products
        if(abs(xmt-sqrt(xm12)).gt.1.d-5)then
          write(*,*)'Error #4 in xmadsubht_dc',xmt,sqrt(xm12)
          stop
        endif
        call gentopdmom(xmt,q1,cthtq1,phitq1,cthtq2,phitq2,
     #                  xtq,xbq,xtl,xtn,1)
        if(ichkmom.eq.0)call checktdec1(xmt,xtq,xbq,xtl,xtn,1)
        if(ichkmom.eq.0)call checktdec2(xmom_cm,4,6,7,8)
c Dummy for Higgs decay (not done here)
        call genhdmom()
        if(ichkmom.eq.0)call checkmom(xmom_cm,sp,ycmp,10,1)
        do ipart=1,7
          do icomp=0,3
            if( jproc.eq.1 .or.
     #          (jproc.eq.2.and.(idr.eq.1.or.idr.eq.3.or.
     #                          idr.eq.5.or.idr.eq.7)) .or.
     #          (jproc.eq.3.and.idr.eq.1) )then
              pme1dc(icomp,ipart)=xmom_cm(mapdir(ipart),mapcomp(icomp))
            else
              pme1dc(icomp,ipart)=xmom_cm(mapref(ipart),mapcomp(icomp))
            endif
          enddo
        enddo
c Restore original kinematics (because of common blocks) 
     ...
     #                tk0,uk0,q1q0,q2q0,xinv0)
        ...
        write(*,*)'xmadsubht_dc: unknown iborn value',iborn
        stop
      endif
      if(iborn.eq.1.and.jproc.eq.1)then
        call ggthb_dc_s(pme1dc,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.1.or.idr.eq.2))then
        call uuxthbx_dc_s(pme1dc,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.5.or.idr.eq.6))then
        call bbxthbx_dc_s(pme1dc,tmp)
      else
        write(*,*)'xmadsubht_dc: Error #3'
        stop
      endif
c Insert sin(theta_W) for e^4 -> gw^4 (due to decay), and divide by A^2+B^2
c (due to production)
      xmadsubht_dc=sthw2**2/apbsq*bwsupp*xfact*tmp(1)/(2*s)
      return
      end


C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE GGTHB_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATGGTHB_DC_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
...
C SF: the original name MATRIX has been replaced by MATGGTHB_DC_S
      REAL*8 FUNCTION MATGGTHB_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   2) = -AMP(   3)-AMP(   4)-AMP(   6)-AMP(   7)-AMP(   8)
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTHB_DC_S =MATGGTHB_DC_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
...
C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE BBXTHBX_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTHBX_DC_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
...
C SF: the original name MATRIX has been replaced by MATBBXTHBX_DC_S
      REAL*8 FUNCTION MATBBXTHBX_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   2) = -AMP(   3)-AMP(   4)
      JAMP(   1) = (0.d0,0.d0)
      JAMP(   2) = -AMP(   3)
      MATBBXTHBX_DC_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTHBX_DC_S =MATBBXTHBX_DC_S+ZTEMP*DCONJG(JAMP(I))/
     #                     DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
...
C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE UUXTHBX_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C MadGraph StandAlone Version
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTHBX_DC_S(P ,NHEL(1,IHEL),JC(1))            
               ANS(IPROC)=ANS(IPROC)+T
               IF (T .NE. 0D0 .AND. .NOT.    GOODHEL(IHEL,IPROC)) THEN
                   GOODHEL(IHEL,IPROC)=.TRUE.
               ENDIF
...
C SF: the original name MATRIX has been replaced by MATUUXTHBX_DC_S
      REAL*8 FUNCTION MATUUXTHBX_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   1) = -AMP(   1)-AMP(   2)
      JAMP(   1) = -AMP(   1)
      MATUUXTHBX_DC_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTHBX_DC_S =MATUUXTHBX_DC_S+ZTEMP*DCONJG(JAMP(I))/
     #                     DENOM(I)   
      ENDDO
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      END
