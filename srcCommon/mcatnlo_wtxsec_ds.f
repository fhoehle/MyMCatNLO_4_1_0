c Identical to mcatnlo_wtxsecs_zw3.f, with the addition of routines
c relevant to the computation of spin correlations with the subtraction
c term. These routines are taken from wtxsec_dec_sub.f
c
c
...
c
c
      subroutine frealwtme(s,x,yi,cth2,tk,uk,q1q,q2q,xinv,
     #                     jproc,xmatout)
...
c
      xii=1-x
      if(jproc.eq.1)then
        call frealwtme_gg(s,xii,yi,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jwt)
          xmatout(i)=xmatin(i)
        enddo
      elseif(jproc.eq.2)then
        call frealwtme_qq(s,xii,yi,cth2,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jwt)
          xmatout(i)=xmatin(i)
        enddo
      elseif(jproc.eq.3)then
        call frealwtme_qg(s,xii,yi,cth2,tk,uk,q1q,q2q,xinv,xmatin)
        do i=1,idrmax(jproc,jwt)
          xmatout(i)=xmatin(i)
        enddo
      else
        write(*,*)'Unknown process in frealwtme',jproc
        stop
      endif
c
      return
      ...
      subroutine frealwtme_gg(xs,xxii,xyi,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for gg --> tWbbar. See frealwtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xtk,xuk,xq1q,xq2q,xxinv(5),
     #  xmatout(7)
      real * 8 s,xii,yi,tk,uk,q1q,q2q,q1c,q2c,tiny,pi,vcf,
     #  s_red,t_red,x_ap,ap_kern,xmadevwt,xinv(5),xmatin(7)
...
c Collinear + limit
        ...
        call fbornwtme(s_red,ymom,ithree,xmatin)
        do i=1,idrmax(ione,jwt)
          if(idrlimcp(jwt,ione,i).ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcp(jwt,ione,i))
...
c use the full expression for the matrix element.
c Factor multiplying the matrix element, eq.(4.37)
        xmatin(1)=xmadevwt(ione,ione,ione,s,tk,uk,xmom_cm)
        do i=1,idrmax(ione,jwt)
          xmatout(i)=xmatin(i)
        enddo
      endif
c
      ...
      subroutine frealwtme_qq(xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for qq --> tWbbar. See frealwtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,xxinv(5),xmatout(7)
      real * 8 s,xii,yi,cth2,tk,uk,q1q,q2q,q1c,q2c,tiny,pi,vcf,
     # s_red,t_red,x_ap,ap_kern,qin_kern,xmadevwt,xinv(5),xmatin(7),
     # xaziin(7)
      real*8 xmom_cm(10,4),ymom(10,4)
      common/cxmomcm/xmom_cm
      integer i,jwt,ione,itwo,ithree,ipone,imone,icode,idrmax(1:3,3)
      common/cidrmax/idrmax
      integer idrlimcp(3,1:3,8),idrlimcm(3,1:3,8)
      common/cidrlims/idrlimcp,idrlimcm
      parameter (tiny=1.d-6)
...
c Collinear + limit
        ...
        call getredmom(1,xmom_cm,ymom,s_red)
        call fbornwtme(s_red,ymom,ithree,xmatin)
        call faziwtme(s_red,t_red,cth2,itwo,ipone,xaziin)
        do i=1,idrmax(itwo,jwt)
          if(idrlimcp(jwt,itwo,i).ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcp(jwt,itwo,i)) +
...
c Collinear - limit
        ...
        call getredmom(-1,xmom_cm,ymom,s_red)
        call fbornwtme(s_red,ymom,ithree,xmatin)
        call faziwtme(s_red,t_red,cth2,itwo,imone,xaziin)
        do i=1,idrmax(itwo,jwt)
          if(idrlimcm(jwt,itwo,i).ne.0)
     #      xmatout(i)=4*(1-yi)/s*ap_kern(x_ap,abs(icode))*
     #                 xmatin(idrlimcm(jwt,itwo,i)) +
...
c Factor multiplying the matrix element, eq.(4.37)
        do i=1,idrmax(itwo,jwt)
          xmatout(i)=xmadevwt(ione,itwo,i,s,tk,uk,xmom_cm)
        enddo
      endif
c
      return
      ...
      subroutine frealwtme_qg(xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,
     #                        xxinv,xmatout)
c Real matrix elements for qg --> tWbbar. See frealwtme for comments
      implicit none
      include 'stpcblks.h'
      real * 8 xs,xxii,xyi,xcth2,xtk,xuk,xq1q,xq2q,xxinv(5),
     # xmatout(7)
     ...
     # softfc(3),xaziin(7)
      real*8 xmom_cm(10,4),ymom(10,4)
      common/cxmomcm/xmom_cm
      integer i,jwt,ione,ithree,ipone,imone,icode,idrmax(1:3,3)
      common/cidrmax/idrmax
      integer iapcp(3,7),iapcm(3,7)
      common/ciap/iapcp,iapcm
      integer idrlimcp(3,1:3,8),idrlimcm(3,1:3,8)
...
c Collinear + limit
        ...
        call getredmom(1,xmom_cm,ymom,s_red)
        call fbornwtme(s_red,ymom,ithree,xmatin)
        call faziwtme(s_red,t_red,cth2,ithree,ipone,xaziin)
        do i=1,idrmax(ithree,jwt)
          icode=iapcp(jwt,i)
          if(idrlimcp(jwt,ithree,i).ne.0.and.icode.ne.0)
     #      xmatout(i)=4*(1+yi)/s*ap_kern(x_ap,abs(icode))*
...
c Collinear - limit
        ...
        call getredmom(-1,xmom_cm,ymom,s_red)
        call fbornwtme(s_red,ymom,ithree,xmatin)
        call faziwtme(s_red,t_red,cth2,ithree,imone,xaziin)
        do i=1,idrmax(ithree,jwt)
          icode=iapcm(jwt,i)
          if(idrlimcm(jwt,ithree,i).ne.0.and.icode.ne.0)
     #      xmatout(i)=4*(1-yi)/s*ap_kern(x_ap,abs(icode))*
...
c Factor multiplying the matrix element, eq.(4.37)
        do i=1,idrmax(ithree,jwt)
          if(i.ne.2)xmatout(i)=xmadevwt(ione,ithree,i,s,tk,uk,xmom_cm)
        enddo
      endif
      return
      end
      ...
      subroutine fbornwtme(s,xmom,jproc,xmatout)
c Returns the partonic Born contribution, with the following normalization
c
...
c   u=(p1-k2)^2=(p2-k1)^2
      ...
      parameter (zero=0.d0)
      integer jproc,i,izero,ione,ithree,jwt,idrmax(1:3,3)
      common/cidrmax/idrmax
      parameter (izero=0)
      parameter (ione=1)
      parameter (ithree=3)
...
c
      if(jproc.lt.1.or.jproc.gt.3)then
        write(*,*)'Error in fbornwtme: jproc=',jproc
        stop
      endif
c
      do i=1,idrmax(jproc,jwt)
...
c
      if(jproc.eq.3)then
        xmatout(1)=xmadevwt(izero,ithree,ione,s,zero,zero,xmom)
        xmatout(3)=xmadevwt(izero,ithree,ithree,s,zero,zero,xmom)
      endif
c
      return
      end
      ...
      function azidepme(s,t,u,m2,mw2,cth2)
c Returns the azimuthal-dependent part for the y=-1 collinear singularity
c of any real process that factorizes
c  b(p1)+g(p2) --> t(k1)+W(k2) 
...
c Identical to azidep(); the string me is appended to avoid clashing names
      implicit none
      real * 8 azidepme,s,t,u,m2,mw2,cth2
c
      azidepme = -8*(2*cth2**2-1)*(mw2-m2)*(2*mw2+m2)*(m2*mw2-t*u)/
     1   (mw2*s*(u-m2)**2)
      return 
      end 


      subroutine faziwtme(s,t,cth2,jproc,icoll,xmatout)
c Returns the azimuthal-dependent part of collinear limits; the definitions
c are as in fbornwt.
c Identical to faziwt(); the string me is appended to avoid clashing names
      implicit none
      include 'stpcblks.h'
      real*8 s,t,cth2,xmatout(7)
...
c
      if(jproc.lt.2.or.jproc.gt.3)then
        write(*,*)'Error in faziwtme: jproc=',jproc
        stop
      endif
c
      do i=1,idrmax(jproc,jwt)
...
c
      ...
      elseif(icoll.eq.-1)then
        ...
        write(*,*)'Error in faziwtme: icoll=',icoll
        stop
      endif
c
      return
...
c to g^2 gw^2 (g^4 gw^2), and this call can be placed somewhere
c else (and done only once). The main code assumes that g^n gw^2 be
...
c MadEvent must be multiplied by sthw2=e^2/gw^2
      implicit none
...
c Subprocess: MC@NLO   -> x(1)y(2)  ->  z(3)t(4)W(5)
c             MadEvent -> b(1)g(2)  ->  t(3)W(4)
c             MadEvent -> g(1)g(2)  ->  t(3)W(4)b(5)
c             MadEvent -> b(1)u(2)  ->  t(3)W(4)u(5)
c             MadEvent -> b(1)bx(2) ->  t(3)W(4)bx(5)
c             MadEvent -> b(1)g(2)  ->  t(3)W(4)g(5)
...
c
     ...
     #       (idr.eq.5.or.idr.eq.6))then
      ...
      xmadevwt=sthw2*xfact*tmp(1)/(2*s)
      return
      end
...
C SF: the original name MATRIX has been replaced by MATBGTW
      REAL*8 FUNCTION MATBGTW(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,TMASS   ,ZERO  ,W(1,5   ))     
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          MATBGTW =MATBGTW+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
...
C SF: the original name MATRIX has been replaced by MATGGTWB
      REAL*8 FUNCTION MATGGTWB(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C  
      ...
      COMPLEX*16 AMP(NGRAPHS), JAMP(NCOLOR)
      COMPLEX*16 W(6,NWAVEFUNCS)
C SF: need introduce temporary wavefunctions, since some of the original ones
C SF: are used both for ttbar and for Wt amplitudes, which have different 
C SF: top widths here
      COMPLEX*16 W6(6),W9(6)
C  
C GLOBAL VARIABLES
C  
C SF: comment out all instances of amp2, used by multi_channel
...
C ----------
      ...
      CALL FVOXXX(W(1,6   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,7   ))    
      CALL IOVXXX(W(1,5   ),W(1,7   ),W(1,1   ),GG ,AMP(1   ))             
      CALL FVOXXX(W(1,3   ),W(1,2   ),GG ,TMASS   ,TWIDTH  ,W6(1    ))     
      CALL FVOXXX(W6(1    ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W(1,8   ))     
      CALL IOVXXX(W(1,5   ),W(1,8   ),W(1,4   ),GWF ,AMP(2   ))            
      CALL FVOXXX(W(1,3   ),W(1,1   ),GG ,TMASS   ,ZERO  ,W(1,9   ))     
      CALL FVOXXX(W(1,9   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,10  ))    
      CALL IOVXXX(W(1,5   ),W(1,10  ),W(1,2   ),GG ,AMP(3   ))             
      CALL FVOXXX(W(1,3   ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W9(1    ))     
      CALL FVOXXX(W9(1    ),W(1,2   ),GG ,TMASS   ,TWIDTH  ,W(1,11  ))     
      CALL IOVXXX(W(1,5   ),W(1,11  ),W(1,4   ),GWF ,AMP(4   ))            
      CALL FVOXXX(W(1,3   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,12  ))    
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTWB =MATGGTWB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
C SF: the original name MATRIX has been replaced by MATBGTWG
      REAL*8 FUNCTION MATBGTWG(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FVIXXX(W(1,1   ),W(1,4   ),GWF ,TMASS   ,ZERO  ,W(1,8   ))    
      ...
      CALL FVOXXX(W(1,3   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,11  ))    
      ...
      CALL FVIXXX(W(1,13  ),W(1,4   ),GWF ,TMASS   ,ZERO  ,W(1,14  ))    
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBGTWG =MATBGTWG+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTWBX(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATBBXTWBX
      REAL*8 FUNCTION MATBBXTWBX(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL VXXXXX(P(0,4   ),WMASS ,NHEL(4   ),+1*IC(4   ),W(1,4   ))       
      CALL IXXXXX(P(0,5   ),BMASS ,NHEL(5   ),-1*IC(5   ),W(1,5   ))       
      CALL FVIXXX(W(1,1   ),W(1,4   ),GWF ,TMASS   ,ZERO  ,W(1,6   ))    
      CALL JIOXXX(W(1,5   ),W(1,2   ),GG ,ZERO    ,ZERO    ,W(1,7   ))     
      CALL IOVXXX(W(1,6   ),W(1,3   ),W(1,7   ),GG ,AMP(1   ))             
      CALL FVOXXX(W(1,3   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,8   ))    
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTWBX =MATBBXTWBX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
C SF: the original name MATRIX has been replaced by MATBUTWU
      REAL*8 FUNCTION MATBUTWU(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBUTWU =MATBUTWU+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
C SF: the original name MATRIX has been replaced by MATBBTWB
      REAL*8 FUNCTION MATBBTWB(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FVOXXX(W(1,3   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,8   ))    
      CALL IOVXXX(W(1,1   ),W(1,8   ),W(1,7   ),GG ,AMP(2   ))             
      CALL FVIXXX(W(1,2   ),W(1,4   ),GWF ,TMASS   ,ZERO  ,W(1,9   ))    
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBTWB =MATBBTWB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTWBX(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATUUXTWBX
      REAL*8 FUNCTION MATUUXTWBX(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTWBX =MATUUXTWBX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
c
     ...
     #      ymom(1,2)*ymom(2,2)-ymom(1,3)*ymom(2,3) )
      ...
      subroutine frealwtsub(jproc,xm12,xm22,s,x,y,cth1,cth2,
     #                      tk,uk,q1q,q2q,x1,x2,x1p,x2p,xmatout)
c Wrapper for xmadsubwt. See comments there
      implicit none
      real*8 xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,x1,x2,
     # x1p,x2p,xmadsubwt,xmatout(7)
...
c
      ...
      enddo
      if(jproc.eq.1)then
        xmatout(1)=xmadsubwt(iborn,jproc,ione,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
      elseif(jproc.eq.2)then
        xmatout(1)=xmadsubwt(iborn,jproc,ione,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(2)=xmadsubwt(iborn,jproc,itwo,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(5)=xmadsubwt(iborn,jproc,ifive,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
        xmatout(6)=xmadsubwt(iborn,jproc,isix,xm12,xm22,s,x,y,
     #                       cth1,cth2,tk,uk,q1q,q2q,x1,x2,x1p,x2p)
      else
        x1p=1.d0
        x2p=1.d0
      ...
      function xmadsubwt(iborn,jproc,idr,xm12,xm22,s,x,y,cth1,cth2,
     #                   tk,uk,q1q,q2q,x1,x2,x1p,x2p)
c Computes the gg->tWbbar and qqbar/bbbar->tWbbar contributions, including
c only the doubly-resonant diagrams. This is done by assuming the 
c processes above are actually gg->t(tbar->Wbbar). The original kinematics
c is reshuffled in such a way that t and tbar are both on shell. The
c decay tbar->Wbbar is performed in the same manner as done in the
...
c gg->tbarWb and qqbar/bbbar->tbarWb. In such a case the decay is t->Wb.
...
c This routine is derived from xmadevwt
      implicit none
      integer iborn,jproc,idr
      real*8 xmadsubwt,xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,
...
c
      if(iborn.eq.0)then
        write(*,*)'Error #1 in xmadsubwt: iborn, jproc=',iborn,jproc
        stop
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.2.and.
     ...
     #      jproc.eq.3 )then
          write(*,*)'Error #2 in xmadsubwt: jproc,idr=',jproc,idr
          stop
        endif
c FKS damping factor
        xfact=4*tk*uk/s**2
c bW invariant mass -- ie off-shell top virtuality
        w2=xm22-q2q+q1q-uk
...
c Restore original kinematics (because of common blocks) 
     ...
     #                tk0,uk0,q1q0,q2q0,xinv0)
      ...
      if(iborn.eq.1.and.jproc.eq.1)then
        call ggtwb_s(pme1,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.1.or.idr.eq.2))then
        call uuxtwbx_s(pme1,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.5.or.idr.eq.6))then
        call bbxtwbx_s(pme1,tmp)
      else
        write(*,*)'xmadsubwt: Error #3'
        stop
      endif
      xmadsubwt=sthw2*bwsupp*xfact*tmp(1)/(2*s)
      return
      end
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATGGTWB_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATGGTWB_S
      REAL*8 FUNCTION MATGGTWB_S(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   2) = -AMP(   3)-AMP(   4)-AMP(   6)-AMP(   7)-AMP(   8)
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTWB_S =MATGGTWB_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTWBX_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATBBXTWBX_S
      REAL*8 FUNCTION MATBBXTWBX_S(P,NHEL,IC)
C  
C Generated by MadGraph II Version 3.90. Updated 08/26/05               
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL VXXXXX(P(0,4   ),WMASS ,NHEL(4   ),+1*IC(4   ),W(1,4   ))       
      CALL IXXXXX(P(0,5   ),BMASS ,NHEL(5   ),-1*IC(5   ),W(1,5   ))       
      CALL FVIXXX(W(1,1   ),W(1,4   ),GWF ,TMASS   ,TWIDTH  ,W(1,6   ))    
      CALL JIOXXX(W(1,5   ),W(1,2   ),GG ,ZERO    ,ZERO    ,W(1,7   ))     
      CALL IOVXXX(W(1,6   ),W(1,3   ),W(1,7   ),GG ,AMP(1   ))             
      CALL FVOXXX(W(1,3   ),W(1,4   ),GWF ,BMASS   ,ZERO    ,W(1,8   ))    
      CALL IOVXXX(W(1,1   ),W(1,8   ),W(1,7   ),GG ,AMP(2   ))             
...
c$$$      JAMP(   2) = -AMP(   3)-AMP(   4)
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTWBX_S =MATBBXTWBX_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTWBX_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATUUXTWBX_S
      REAL*8 FUNCTION MATUUXTWBX_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   1) = -AMP(   1)-AMP(   2)
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTWBX_S =MATUUXTWBX_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      ...
      function xmadevwt_dc(iborn,jproc,idr,s,tk,uk,xmom)
c Wrapper for MadEvent functions. Inputs are 
c   iborn = 0(born), 1(real)
c   jproc = 1(gg), 2(qq), 3(qg)
c   idr   = 1..7 (depends on jproc)
...
c MadEvent must be multiplied by sthw2^4=e^8/gw^8
      implicit none
      integer iborn,jproc,idr
      real*8 xmadevwt_dc,s,tk,uk,xmom(10,4),xfact,tmp(1)
...
c
      if(iborn.eq.0)then
        if(jproc.ne.3)then
          write(*,*)'Error #1 in xmadevwt_dc: iborn, jproc=',
     #              iborn,jproc
          stop
        endif
        xfact=1.d0
        do ipart=1,7
          do icomp=0,3
            ...
            elseif(idr.eq.3)then
              ...
              write(*,*)'Error #2 in xmadevwt_dc: idr=',idr
              stop
            endif
          enddo
        enddo
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.3.and.(idr.ne.1.and.idr.ne.3)) )then
          write(*,*)'Error #3 in xmadevwt_dc: jproc,idr=',jproc,idr
          stop
        endif
        xfact=4*tk*uk/s**2
        do ipart=1,8
     ...
     #          (jproc.eq.3.and.idr.eq.1) )then
        ...
        write(*,*)'xmadevwt_dc: unknown iborn value',iborn
        stop
      endif
      if(iborn.eq.0)then
     ...
     #       (idr.eq.5.or.idr.eq.6))then
      ...
      elseif(iborn.eq.1.and.jproc.eq.3)then
        ...
        write(*,*)'xmadevwt_dc: Error #5'
        stop
      endif
c Insert sin(theta_W) for e^8 -> gw^8
      xmadevwt_dc=sthw2**4*xfact*tmp(1)/(2*s)
      return
      end
...
C      CALL GAUGECHECK(JAMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NCOLOR,NEIGEN) 
      ...
      function xmadsubwt_dc(iborn,jproc,idr,xm12,xm22,s,x,y,cth1,cth2,
     #                      tk,uk,q1q,q2q,x1,x2,x1p,x2p,
...
c This routine is derived from xmadevwt
      implicit none
      integer iborn,jproc,idr
      real*8 xmadsubwt_dc,xm12,xm22,s,x,y,cth1,cth2,tk,uk,q1q,q2q,
     # x1,x2,x1p,x2p,xfact,w2,bwsupp,bwfunc,sp,staup,xlstaup,ycmp,
     # tk0,uk0,q1q0,q2q0,one,cth1p,cth2p,cthbw,phibw,xinv0(5),
     # pme1dc(0:3,8),tmp(1)
      real*8 q1,cthtq1,phitq1,cthtq2,phitq2,cthwl,phiwl
      real*8 xtq(4),xbq(4),xtl(4),xtn(4)
      real*8 xwb(4),xwl(4),xwn(4)
      parameter (one=1.d0)
      character*2 str
      parameter (str='p1')
      real*8 xmom_cm(10,4)
...
c
      if(iborn.eq.0)then
        write(*,*)'Error #1 in xmadsubwt_dc: iborn, jproc=',iborn,jproc
        stop
      elseif(iborn.eq.1)then
        if( (jproc.eq.1.and.idr.ne.1) .or.
     #      (jproc.eq.2.and.
     #       (idr.ne.1.and.idr.ne.2.and.idr.ne.5.and.idr.ne.6)) .or.
     #      jproc.eq.3 )then
          write(*,*)'Error #2 in xmadsubwt_dc: jproc,idr=',jproc,idr
          stop
        endif
c FKS damping factor
        xfact=4*tk*uk/s**2
...
c Generate momenta of top decay products
        if(abs(xmt-sqrt(xm12)).gt.1.d-5)then
          write(*,*)'Error #4 in xmadsubwt_dc',xmt,sqrt(xm12)
          stop
        endif
...
c Generate momenta of hard W decay products
        if(abs(xmw-sqrt(xm22)).gt.1.d-5)then
          write(*,*)'Error #5 in xmadsubwt_dc',xmw,sqrt(xm22)
          stop
        endif
        call genwdmom(xmw,cthwl,phiwl,xwb,xwl,xwn,2)
        if(ichkmom.eq.0)call checkwdec1(xmw,xwb,xwl,xwn,2)
        if(ichkmom.eq.0)call checkwdec2(xmom_cm,5,9,10)
...
c Restore original kinematics (because of common blocks) 
     ...
     #                tk0,uk0,q1q0,q2q0,xinv0)
        ...
        write(*,*)'xmadsubwt_dc: unknown iborn value',iborn
        stop
      endif
      if(iborn.eq.1.and.jproc.eq.1)then
        call ggtwb_dc_s(pme1dc,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.1.or.idr.eq.2))then
        call uuxtwbx_dc_s(pme1dc,tmp)
      elseif(iborn.eq.1.and.jproc.eq.2 .and.
     #       (idr.eq.5.or.idr.eq.6))then
        call bbxtwbx_dc_s(pme1dc,tmp)
      else
        write(*,*)'xmadsubwt_dc: Error #3'
        stop
      endif
c Insert sin(theta_W) for e^8 -> gw^8
      xmadsubwt_dc=sthw2**4*bwsupp*xfact*tmp(1)/(2*s)
      return
      end


C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE GGTWB_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C AND HELICITIES
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATGGTWB_DC_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATGGTWB_DC_S
      REAL*8 FUNCTION MATGGTWB_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   2) = +AMP(   3)+AMP(   4)+AMP(   6)+AMP(   7)+AMP(   8)
      ...
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATGGTWB_DC_S =MATGGTWB_DC_S+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
c          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
...
C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE BBXTWBX_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C AND HELICITIES
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATBBXTWBX_DC_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATBBXTWBX_DC_S
      REAL*8 FUNCTION MATBBXTWBX_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   2) = +AMP(   3)+AMP(   4)
      JAMP(   1) = (0.d0,0.d0)
      JAMP(   2) = +AMP(   4)
      MATBBXTWBX_DC_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATBBXTWBX_DC_S =MATBBXTWBX_DC_S+
...
C SF: The following routine is SMATRIX generated by MadEvent, suitably modified
      SUBROUTINE UUXTWBX_DC_S(P1,ANS)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C AND HELICITIES
...
c      ENDIF
      ...
      IF (ISUM_HEL .EQ. 0 .OR. NTRY .LT. 10) THEN
          DO IHEL=1,NCOMB
              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MATUUXTWBX_DC_S(P ,NHEL(1,IHEL),JC(1))            
                 ANS(IPROC)=ANS(IPROC)+T
                  IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL,IPROC)) THEN
                      GOODHEL(IHEL,IPROC)=.TRUE.
                      NGOOD = NGOOD +1
...
C SF: the original name MATRIX has been replaced by MATUUXTWBX_DC_S
      REAL*8 FUNCTION MATUUXTWBX_DC_S(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
c$$$      JAMP(   1) = +AMP(   1)+AMP(   2)
      JAMP(   1) = +AMP(   2)
      MATUUXTWBX_DC_S = 0.D0 
      DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATUUXTWBX_DC_S =MATUUXTWBX_DC_S+
     #                       ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C SF: comment out all instances of amp2, used by multi_channel
c      Do I = 1, NGRAPHS
